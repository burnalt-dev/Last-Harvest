...
export function drawDungeon(
  ...
  for (const t of order) {
    ...
    if (tiles) {
      const floorCol = tile === 3 || tile === 1 ? 1 : 0;
      const floorRow = tile === 3 || tile === 2 ? 1 : 0;
      const isStairs = tile === 2;
      blitCell(ctx, tiles, isStairs ? 0 : floorCol, isStairs ? 1 : floorRow, 2, 2, p.x, p.y, dw, dh);
    } else diamond(ctx, p.x, p.y, tile === 1 ? "#565044" : tile === 2 ? "#2a1c14" : "#8a6e4a", "#1a1410");
    corners(ctx, p.x, p.y);
    if (tile === 2) {
