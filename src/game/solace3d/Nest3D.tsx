...
import { faceSmall, occSmoke } from "../grid";
import { DIR } from "../iso";
...
export function Nest3D({ onBack }: { onBack: () => void }) {
  ...
  const [occLine, setOccLine] = useState("");
  const [pinBefore, setPinBefore] = useState("");
  const [pinAfter, setPinAfter] = useState("");
  const [pinHit, setPinHit] = useState("");
  ...
  useEffect(() => {
    ...
    const reportOcc = (tag: "SPAWN" | "STEP") => {
      const smoke = occSmoke(
        actors.map((a) => ({ id: a.uid, x: a.x, y: a.y, sub: a.sub, hp: a.hp, small: a.small })),
        tag,
      );
      setOccLine(smoke.line);
      setPackHp(nestPackHp(actors));
      setBodyLine(nestBodyHp(actors));
    };
    const reportLance = () => {
    ...
    const plantPack = async (row: NestFoe, n: number, token: number) => {
      ...
      reportOcc("SPAWN");
      setPinBefore(nestPinLine(actors));
      setPinAfter("");
      setPinHit("");
    };
    ...
    api.current = {
      ...
      shoot: () => {
        if (!nestCanLance(lanceLive.stance)) return;
        const fx: Fx[] = [];
        const plate = nestShoot(actors, gasketLive, lanceLive, fx);
        setPinBefore(plate.before);
        setPinAfter(plate.after);
        setPinHit(plate.hit);
        faceWanzer();
        plantAim();
        reportLance();
      ...
      stepGasket: (dir) => {
        const d = DIR[dir]!;
        nestGasketStep(actors, gasketLive, d.x, d.y);
        plantGasket();
        reportHp();
        reportOcc("STEP");
      },
      stepFoe: () => {
        nestStepFoe(actors, gasketLive);
        syncActors();
        reportOcc("STEP");
      },
    };
    ...
    const tick = (now: number) => {
      ...
      if (fight.on && actors.length && now - lastStep >= NEST_TICK_MS) {
        lastStep = now;
        nestRound(actors, gasketLive, now, fight.cleave && !nestCanLance(lanceLive.stance), gasketLive.kit === "harvester");
        syncActors();
        plantGasket();
        plantAim();
        reportHp();
  ...
  return (
    <main className="relative z-[80] h-dvh overflow-y-auto bg-bg" data-qa="nest3d">
      ...
      <section className="border-t border-border px-3 py-2">
        ...
        <p className="text-[10px] text-muted" data-qa="nest-pin-before">
          {pinBefore || "—"}
        </p>
        <p className="text-[10px] text-muted" data-qa="nest-pin-after">
          {pinAfter || "—"}
        </p>
        <p className="text-[10px] text-muted" data-qa="nest-pin-hit">
          {pinHit || "—"}
        </p>
        ...
        <p className="mt-1 break-all text-[10px] text-muted" data-qa="nest-occ">
          {occLine || "OCC — spawn a pack"}
        </p>
      </section>
      <section className="border-t border-border px-3 py-2">
        ...
        <p className="engraved mt-3 mb-1 text-[10px]">Move (¼ Gasket · Stripekin)</p>
        <div className="mb-2 grid grid-cols-3 gap-2" data-qa="nest-move">
          <span />
          <Plate qa="nest-move-n" onClick={() => api.current?.stepGasket(2)}>
            N
          </Plate>
          <span />
          <Plate qa="nest-move-w" onClick={() => api.current?.stepGasket(3)}>
            W
          </Plate>
          <Plate qa="nest-step" onClick={() => api.current?.stepFoe()}>
            Step ¼
          </Plate>
          <Plate qa="nest-move-e" onClick={() => api.current?.stepGasket(1)}>
            E
          </Plate>
          <span />
          <Plate qa="nest-move-s" onClick={() => api.current?.stepGasket(0)}>
            S
          </Plate>
          <span />
        </div>
        ...
        <p className="mt-2 text-[10px] text-muted">Gasket NESW = ¼. Step ¼ = one Stripekin. Pilot stays full-tile.</p>
      </section>
      {spec ? (
        <section className="border-t border-border px-3 py-2">
