...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Chrome is rivet **plates** (`last-harvest-shell` / `Plate.tsx`), not shadcn. Bay canvas is 3D; HUD over the drop is 2D. Player stat is **Wits** (`wits`), never Perception.
