...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Crate identity is **3D sockets** (yaw, one mesh). Gasket is one cute mesh (`gasket.ts`) — not modular. Foes are whole **3D** `buildFoe` meshes. Do not isolate orc/Gasket limbs as `p###`.
