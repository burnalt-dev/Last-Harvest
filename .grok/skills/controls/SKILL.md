...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Dungeon + Bay pad is Front Mission **iso diamond** (`DIR` + `ISO_YAW` ±45°, S58). Not FPS strafe. Not a chase-cam vehicle. Open `last-harvest-3d` before changing Bay walk. Small units: `last-harvest-grid` ¼-square, not crate 1:1.
