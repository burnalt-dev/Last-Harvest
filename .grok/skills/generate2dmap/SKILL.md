...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Veldt floors are **2D iso diamonds** via `last-harvest-terrain`. Never WebGL the dungeon (`iso3d` dropped). Not crate art. Phaser/Unity exports are **wrong** for this game.
