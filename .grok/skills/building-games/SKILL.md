...
**Last Harvest override:** open `last-harvest` then **`last-harvest-makers`** (`LAST HARVEST MAKER SKILLS — FOLLOW BEFORE ANY CHANGE`) before any change. Solace crate + Gasket + foes = **3D three.js** (`solace3d/`, no R3F, no Phaser, no Unity). Terrain / fog / props = **2D iso textures** on occupancy diamonds. Do not Imagine 4-dir crate parts. Do not mesh the Veldt floor.
