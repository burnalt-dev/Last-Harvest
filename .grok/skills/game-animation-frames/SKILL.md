...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Crate clips are **3D joints** (`solace-mech-anim` / `kit.ts`). Do not Imagine 4-dir walk cycles for Solace. Gasket idle is `gasket.ts` tick.
