...
**Last Harvest:** open `last-harvest` then **`last-harvest-makers`**. Bay and Drop3D are raw `THREE.WebGLRenderer` in `src/game/solace3d/` — **no R3F, no drei**. Terrain stays 2D textures on occupancy diamonds. Do not swap the engine.
