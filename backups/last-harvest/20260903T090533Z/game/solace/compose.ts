...
function stackSockets(dest: CanvasRenderingContext2D, loadout: Loadout, dir: Dir, pose: string, bobY: number) {
  dest.imageSmoothingEnabled = false;
  dest.clearRect(0, 0, CELL, CELL);
  ...
  for (const step of BLIT_ORDER) {
    ...
    const fr =
      frames.find((f) => f.pose === pose && f.dir === dir) ??
      frames.find((f) => f.pose === "idle" && f.dir === dir);
    if (!fr) continue;
    seen.add(id);
    const pt = sock[step.socket];
    const extra = step.socket === "hip" || step.socket === "neck" || step.socket.startsWith("shoulder") ? bobY : 0;
    const dx = (pt.x - fr.origin.x) | 0;
    const dy = (pt.y - fr.origin.y + extra) | 0;
