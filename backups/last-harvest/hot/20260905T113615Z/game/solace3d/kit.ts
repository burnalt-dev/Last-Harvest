...
export const GOLD_GREY: Kit = { head: 0, body: 0, legs: 0, arms: 0, handR: 0, handL: 0 };
...
export const KIT_NAMES: Record<KitSlot, string[]> = {
  ...
  handL: ["none", "shield", "cannon"],
};

/** Scythe zeros off-hand. Bay may show Lancers without a cell mesh (pack is not visible). */
export function clampKit(k: Kit): Kit {
  const next = { ...k };
  if (KIT_NAMES.handR[next.handR] === "scythe") next.handL = 0;
  return next;
}
...
export function kitFromLoadout(l: {
  ...
  const twoHand = l.handR === "p018" || l.handR === "p009";
  const handR = twoHand ? 1 : l.handR === "p008" ? 2 : l.handR === "p010" ? 3 : 0;
  const handL = twoHand ? 0 : l.handL === "p007" ? 1 : l.handL === "p010" ? 2 : 0;
  return clampKit({
    head: l.head ? 1 : 0,
...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  if (rKind !== "cannon") {
    const hold = rKind === "scythe" ? "pole" : rKind === "saw" ? "hilt" : "open";
    armR.wrist.add(buildGripper(1, hold));
  }
  if (lKind !== "cannon") {
    const hold = rKind === "scythe" ? "pole" : lKind === "shield" ? "slab" : "open";
    armL.wrist.add(buildGripper(-1, hold));
  }
  ...
  let swinging = false;
  /** Bind +Y is up. Hang = π. Lance present = elbow −π/2. Scythe = gold farmer-cut (both fists on the snath). */
  const reap = rKind === "scythe";
  const hangU = Math.PI - 0.28;
  const hangE = 0.2;
  const presentU = Math.PI - 0.12;
  const presentE = -Math.PI / 2;
  const rest = {
    chestX: 0,
    upperL: reap ? 0.58 : lKind === "cannon" ? presentU : hangU,
    upperR: reap ? 0.55 : rKind === "cannon" ? presentU : hangU,
    elbowL: reap ? 0.72 : lKind === "cannon" ? presentE : hangE,
    elbowR: reap ? 0.78 : rKind === "cannon" ? presentE : hangE,
    zL: reap ? 0 : lKind === "cannon" ? 0.12 : 0.06,
    zR: reap ? 0 : rKind === "cannon" ? -0.12 : -0.06,
  };
  ...
  if (lKind === "shield") {
    armL.forearm.rotation.y = -0.45;
    armL.wrist.rotation.z = 0.08;
  }
  if (rKind === "saw") armR.wrist.rotation.z = 0.1;
  const swingDir = reap ? 1 : -1;
  ...
  const swing = () => {
    ...
    const tickSw = (now: number) => {
      ...
      armR.upper.rotation.x = rest.upperR + swingDir * lift;
      armL.upper.rotation.x = rest.upperL + swingDir * lift * 0.7;
