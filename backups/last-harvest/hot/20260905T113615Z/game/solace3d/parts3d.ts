...
export type GripHold = "open" | "pole" | "hilt" | "slab";

/** Universal labor clamp. Lives on `wrist_*`. Cannon is the hand — no gripper on a gun arm. */
export function buildGripper(side: 1 | -1, hold: GripHold): THREE.Group {
  const g = new THREE.Group();
  g.name = side < 0 ? "gripper_L" : "gripper_R";
  const charcoal = texMat("charcoal");
  const brass = texMat("brass");
  const dark = texMat("dark");
  const curl = hold === "open" ? 0.38 : 1.05;
  ...
  else if (hold === "hilt") g.rotation.set(0.08, 0.12, 0.1);
  return g;
}
...
function buildCannon(side: 1 | -1 = 1): THREE.Group {
  ...
  gun.name = "cannon";
  const cuff = cyl(0.05, 0.044, 0.08, dark, 8);
