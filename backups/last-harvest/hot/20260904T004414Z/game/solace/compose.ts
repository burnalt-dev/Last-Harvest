...
function destOf(x: number, y: number, span: number, cx: number, top: number) {
  const base = (span * 156) | 0;
  const targetH = FOOT - 20;
  const h = Math.max(64, FOOT - top);
  let k = targetH / h;
  if (k > 1.1) k = 1.1;
  if (k < 0.92) k = 0.92;
  const dw = (base * k) | 0;
...
export function drawComposite(
  ...
  const { dw, dh, x0, y0 } = destOf(x, y, span, rec.cx, rec.top);
  ctx.imageSmoothingEnabled = false;
...
export function companionMountXY(dir: number, x: number, y: number, span: number, loadout?: Loadout) {
  ...
  const rec = loadout ? compositeOf(loadout, dir, "idle", 0) : { cx: CELL / 2, foot: FOOT, top: 20 };
  const { dw, dh, x0, y0 } = destOf(x, y, span, rec.cx, rec.top);
  return {
