...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  const wpnKind = KIT_NAMES.weapon[kit.weapon] ?? "none";
  if (headKind !== "none") headSlot.add(buildHead(headKind, sit));
  if (bodyKind !== "none") {
    bodySlot.add(buildBody(bodyKind, sit));
  ...
  if (wpnKind !== "none") wpnSlot.add(buildWeapon(wpnKind, sit));
  ...
  let swinging = false;
  const armed = wpnKind !== "none";
  const rest = {
    chestX: 0,
    upperL: armed ? 0.58 : 0.35,
    upperR: armed ? 0.55 : 0.35,
    elbowL: armed ? 0.72 : 0.4,
    elbowR: armed ? 0.78 : 0.4,
  };
