...
import { makePulseOrb, spawnPulse, tickPulses, type PulseShot } from "./pulse";
...
export function Drop3D({
  ...
  useEffect(() => {
    ...
    scene.add(gasket);
    const aimMark = makePulseOrb();
    aimMark.name = "aimLock";
    aimMark.scale.setScalar(0.55);
    ...
    const sync = () => {
      ...
      if (d.aiming && d.aimX >= 0 && d.aimY >= 0) {
        ...
        aimMark.position.set(p.x, 0.28, p.z);
    ...
    const loop = () => {
      ...
      if (aimOn) {
        const bob = 0.5 + Math.sin(now / 180) * 0.08;
        aimMark.scale.setScalar(bob);
        aimMark.position.y = 0.28 + Math.sin(now / 220) * 0.04;
      }
      for (const f of fxRef.current) {
        ...
        const end =
          aimOn && dNow.aimX === f.x1 && dNow.aimY === f.y1 ? dest : isoCell(f.x1, f.y1);
        if (tip) {
          tip.getWorldPosition(tmpA);
