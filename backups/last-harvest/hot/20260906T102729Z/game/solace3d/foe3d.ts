...
/** Barkback — full-tile walking bunker. Shield sponge, not Antlerkin maul. */
function barkback(): THREE.Group {
  ...
  for (const x of [-0.09, 0.09] as const) {
    const thigh = cyl(0.05, 0.042, 0.13, soot, 6);
    thigh.position.set(x, 0.11, 0.02);
    bob.add(thigh);
    const shin = cyl(0.036, 0.032, 0.08, hide, 6);
    shin.position.set(x, 0.035, 0.05);
    bob.add(shin);
    const foot = box(0.08, 0.03, 0.11, soot);
    foot.position.set(x, 0.012, 0.07);
    bob.add(foot);
    for (const [ox, oz] of [
      [-0.02, 0.12],
      [0.02, 0.12],
      ...
      const claw = box(0.014, 0.012, 0.028, cream);
      claw.position.set(x + ox, 0.014, oz);
      bob.add(claw);
    }
  }

  const hip = sph(0.13, hide, 8);
  hip.scale.set(1.5, 0.82, 1.25);
  hip.position.y = 0.2;
  bob.add(hip);
  const belly = sph(0.09, pelt, 7);
  belly.scale.set(1.15, 1.05, 0.72);
  belly.position.set(0, 0.2, 0.11);
  bob.add(belly);

  const shell = sph(0.2, bark, 8);
  shell.scale.set(1.42, 1.08, 1.22);
  shell.position.set(0, 0.34, -0.05);
  bob.add(shell);

  const plates: [number, number, number, number, number, number, number][] = [
    [0, 0.46, -0.02, 0.26, 0.055, 0.18, -0.22],
    [0.12, 0.4, 0.02, 0.16, 0.048, 0.14, -0.18],
    [-0.12, 0.4, 0.02, 0.16, 0.048, 0.14, -0.18],
    [0, 0.3, -0.16, 0.24, 0.05, 0.14, -0.55],
    [0.1, 0.26, -0.1, 0.14, 0.042, 0.12, -0.4],
    [-0.1, 0.26, -0.1, 0.14, 0.042, 0.12, -0.4],
    [0, 0.52, -0.08, 0.18, 0.042, 0.14, -0.12],
    [0.14, 0.34, -0.08, 0.1, 0.04, 0.12, -0.3],
    [-0.14, 0.34, -0.08, 0.1, 0.04, 0.12, -0.3],
  ];
  for (const [x, y, z, w, h, d, rx] of plates) {
    ...
    p.rotation.x = rx;
    bob.add(p);
    const rivet = sph(0.014, scrap, 5);
    rivet.position.set(x * 0.7, y + 0.018, z + d * 0.28);
    bob.add(rivet);
  }

  const mossA = box(0.07, 0.028, 0.055, moss);
  mossA.position.set(0.1, 0.5, -0.02);
  mossA.rotation.z = 0.3;
  bob.add(mossA);
  const mossB = box(0.055, 0.022, 0.045, moss);
  mossB.position.set(-0.08, 0.48, -0.08);
  bob.add(mossB);

  const strap = box(0.04, 0.22, 0.02, dark);
  strap.position.set(0.02, 0.34, 0.14);
  strap.rotation.z = 0.35;
  bob.add(strap);
  const strapB = box(0.035, 0.18, 0.018, dark);
  strapB.position.set(-0.04, 0.3, 0.13);
  ...
  bob.add(head);
  const skull = sph(0.09, soot, 8);
  skull.scale.set(1.05, 0.95, 1.1);
  head.add(skull);
  const blaze = box(0.045, 0.1, 0.03, cream);
  blaze.position.set(0, 0.02, 0.07);
  head.add(blaze);
  const cheekL = sph(0.04, cream, 6);
  cheekL.scale.set(0.8, 0.7, 0.6);
  cheekL.position.set(-0.045, -0.01, 0.05);
  head.add(cheekL);
  const cheekR = sph(0.04, cream, 6);
  cheekR.scale.set(0.8, 0.7, 0.6);
  cheekR.position.set(0.045, -0.01, 0.05);
  head.add(cheekR);
  const snout = cyl(0.04, 0.028, 0.08, cream, 6);
  snout.rotation.x = Math.PI / 2;
  snout.position.set(0, -0.015, 0.1);
  head.add(snout);
  const nose = sph(0.018, soot, 5);
  nose.position.set(0, -0.01, 0.145);
  head.add(nose);
  const eyeL = sph(0.016, ORE, 6);
  eyeL.position.set(-0.035, 0.02, 0.08);
  head.add(eyeL);
  const eyeR = sph(0.016, ORE, 6);
  eyeR.position.set(0.035, 0.02, 0.08);
  head.add(eyeR);
  const earL = box(0.03, 0.04, 0.018, soot);
  earL.position.set(-0.07, 0.07, 0.01);
  earL.rotation.z = 0.4;
  head.add(earL);
  const earR = box(0.03, 0.04, 0.018, soot);
  earR.position.set(0.07, 0.07, 0.01);
  earR.rotation.z = -0.4;
  head.add(earR);

  const armL = cyl(0.038, 0.032, 0.16, hide, 6);
  armL.position.set(-0.18, 0.3, 0.06);
  armL.rotation.z = 0.55;
  armL.rotation.x = -0.25;
  bob.add(armL);
  const shield = new THREE.Group();
  shield.name = "barkback_shield";
  shield.position.set(-0.24, 0.28, 0.14);
  shield.rotation.y = 0.55;
  shield.rotation.x = -0.15;
  bob.add(shield);
  const disc = cyl(0.15, 0.15, 0.045, bark, 10);
  disc.rotation.x = Math.PI / 2;
  shield.add(disc);
  const rim = cyl(0.155, 0.148, 0.02, scrap, 10);
  rim.rotation.x = Math.PI / 2;
  rim.position.z = 0.012;
  shield.add(rim);
  const hex = box(0.07, 0.07, 0.02, ORE);
  hex.rotation.z = Math.PI / 4;
  hex.position.z = 0.028;
  shield.add(hex);
  const xA = box(0.18, 0.018, 0.012, dark);
  xA.rotation.z = 0.6;
  xA.position.z = -0.024;
  shield.add(xA);
  const xB = box(0.18, 0.018, 0.012, dark);
  xB.rotation.z = -0.6;
  xB.position.z = -0.024;
  shield.add(xB);
  for (const a of [0, 1.2, 2.4, 3.6, 4.8] as const) {
    const stud = sph(0.012, brass, 5);
    stud.position.set(Math.cos(a) * 0.11, Math.sin(a) * 0.11, 0.026);
    shield.add(stud);
  }

  const armR = cyl(0.036, 0.03, 0.15, hide, 6);
  armR.position.set(0.18, 0.3, 0.04);
  armR.rotation.z = -0.7;
  armR.rotation.x = -0.15;
  bob.add(armR);
  const mace = new THREE.Group();
  mace.name = "barkback_mace";
  mace.position.set(0.26, 0.22, 0.08);
  mace.rotation.z = -0.35;
  bob.add(mace);
  const haft = cyl(0.016, 0.018, 0.16, bark, 6);
  haft.position.y = 0.04;
  mace.add(haft);
  const headM = box(0.08, 0.08, 0.08, scrap);
  headM.rotation.y = Math.PI / 4;
  headM.position.y = 0.13;
  mace.add(headM);
  const band = box(0.09, 0.02, 0.09, dark);
  band.position.y = 0.1;
  mace.add(band);
  for (const [sx, sy, sz] of [
    ...
    const spike = box(0.018, 0.04, 0.018, scrap);
