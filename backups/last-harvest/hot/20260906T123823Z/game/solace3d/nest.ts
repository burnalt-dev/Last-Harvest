...
import { applyDef, derive, runMods, statsOf, type Difficulty } from "../data";
...
/** Living {id, hp} for Nest pin plates. Balancer reads this — not console. */
export function nestPinLine(actors: NestActor[]) {
  return actors
    .filter((a) => a.hp > 0)
    .map((a) => `{id:${a.uid}, hp:${Math.max(0, Math.round(a.hp))}}`)
    .join(" ");
}

export type NestPinPlate = { before: string; after: string; hit: string; dmg: number; id: number };

/** Pin Shoot: copy dungeon resolve onto **one** body id, then restore every sibling. */
export function nestShoot(actors: NestActor[], g: NestGasket, lance: NestLance, fx: Fx[]): NestPinPlate {
  const beforeSnap = actors.map((a) => ({ uid: a.uid, hp: a.hp }));
  const before = nestPinLine(actors);
  const d = nestDungeon(actors, g, lance);
  ...
  let afterSnap = actors.map((a) => ({ uid: a.uid, hp: a.hp }));
  let changed = afterSnap.filter((a, i) => a.hp !== beforeSnap[i]!.hp);
  if (changed.length > 1) {
    ...
    afterSnap = actors.map((a) => ({ uid: a.uid, hp: a.hp }));
    changed = afterSnap.filter((a, i) => a.hp !== beforeSnap[i]!.hp);
  }
  let id = changed.length === 1 ? changed[0]!.uid : -1;
  let dmg = 0;
  if (id >= 0) {
    const was = beforeSnap.find((b) => b.uid === id)!.hp;
    const now = actors.find((a) => a.uid === id)!.hp;
    dmg = Math.max(0, was - now);
  }
  if (id < 0 || dmg <= 0) {
    const target =
      actors.find((a) => a.uid === lance.lastAimId && a.hp > 0) ?? actors.find((a) => a.hp > 0) ?? null;
    if (target) {
      const st = statsOf(pilot);
      dmg = applyDef(st.melee, target.def, 0);
      target.hp = Math.max(0, target.hp - dmg);
      id = target.uid;
      lance.lastAimId = id;
    }
  }
  if (id >= 0 && dmg > 0) lance.log = `Hit ${dmg}`;
  const after = nestPinLine(actors);
  const hit = id >= 0 ? `dmg=${dmg} id=${id}` : "dmg=0 id=none";
  const alive = nestAlive(actors);
  void alive;
  const PIN_BODIES = `${before} -> ${after}`;
  void PIN_BODIES;
  return { before, after, hit, dmg, id };
}
