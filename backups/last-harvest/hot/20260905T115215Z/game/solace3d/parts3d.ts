...
function buildShield(sit: GoldSit, side: 1 | -1 = 1): THREE.Group {
  ...
  slab.name = "shield";
  const door = box(w, h, 0.036, plate);
  door.position.set(side * w * 0.56, h * 0.19, 0);
  slab.add(door);
  const rim = box(w * 1.12, h * 1.05, 0.014, steel);
  rim.position.set(side * w * 0.56, h * 0.19, 0.018);
  slab.add(rim);
  const slot = box(w * 0.31, h * 0.067, 0.012, dark);
  slot.position.set(side * w * 0.56, h * 0.52, 0.026);
  slab.add(slot);
  const warn = box(w * 0.22, w * 0.22, 0.012, yellow);
  warn.position.set(side * w * 0.56, -h * 0.1, 0.026);
  slab.add(warn);
