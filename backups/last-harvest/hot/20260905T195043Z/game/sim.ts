import { BIOMES, STORY, applyDef, derive, type BiomeId, type Difficulty } from "./data";
...
export function makeDungeon(biome: BiomeId, floor: number, pilot: Pilot, seed = Date.now()): Dungeon {
  ...
  const stats = derive(pilot.stur, pilot.avo, pilot.wits);
  const d: Dungeon = {
    w: 13,
    h: 13,
    floor,
    ...
    cx: 1,
    cy: 1,
    chp: (pilot.difficulty ?? "default") === "story" ? STORY.gasketMax : 22,
    cmax: (pilot.difficulty ?? "default") === "story" ? STORY.gasketMax : 22,
    mounted: false,
    gasketMode: "follow",
    bag: emptyBag(),
    enemies: [],
...
export function reveal(d: Dungeon, pilot: Pilot) {
  const r = derive(pilot.stur, pilot.avo, pilot.wits).radar;
  d.vis.fill(0);
  for (let y = 0; y < d.h; y++)
    for (let x = 0; x < d.w; x++) {
...
function hitPlayer(d: Dungeon, pilot: Pilot, foe: Enemy, fx: Fx[]) {
  const st = derive(pilot.stur, pilot.avo, pilot.wits);
...
function gasketBolt(d: Dungeon, fx: Fx[]) {
  ...
  if (!foe) return;
  const raw = d.difficulty === "story" ? STORY.gasketBolt : 3;
...
function bumpTurn(d: Dungeon, pilot: Pilot, keepCannon = false) {
  ...
  d.turn++;
  const rec = derive(pilot.stur, pilot.avo, pilot.wits).recovery;
  d.energy = Math.min(d.emax, d.energy + rec);
  const patch = d.difficulty === "story" ? STORY.patch : 0;
  d.php = Math.min(d.pmax, d.php + rec * 0.15 + patch);
  if (d.difficulty === "story" && d.chp > 0) {
    d.chp = Math.min(d.cmax, d.chp + STORY.gasketPatch);
  }
  reveal(d, pilot);
}
