...
/**
 * 3/4 iso hanger diamond (same 2:1 blit as Veldt).
 * Plant at isoCell with center (0.5, 0.5) — diamond CENTER = crate feet.
 * S57: never a top-down XZ plate; never center (0.5, 0) (that is the south tip).
 */
export function hangerCell(kind: "pad" | "plate" | "grate") {
  ...
  ctx.restore();
  ctx.strokeStyle = kind === "pad" ? "#c9a227" : "rgba(230, 220, 180, 0.4)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(cx, cy - hh);
  ctx.lineTo(cx + hw, cy);
  ...
  const spr = new THREE.Sprite(mat);
  const aspect = canvas.height / Math.max(1, canvas.width);
  spr.scale.set(Math.SQRT2, Math.SQRT2 * aspect, 1);
