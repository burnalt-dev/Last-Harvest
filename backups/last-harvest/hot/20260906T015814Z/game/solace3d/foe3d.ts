...
function stripekin(): THREE.Group {
  ...
  const hip = sph(0.058, leather, 8);
  hip.scale.set(1.2, 0.75, 1);
  hip.position.y = 0.28;
  g.add(hip);
  const torso = cyl(0.055, 0.065, 0.17, leather, 8);
  torso.position.y = 0.4;
  g.add(torso);
  const rag = box(0.1, 0.12, 0.06, soot);
  rag.position.set(0, 0.4, 0.03);
  g.add(rag);

  /** Scavenged cloth + scrap. Hide stripes only on head / tail. */
  const cuirass = box(0.12, 0.12, 0.08, scrap);
  cuirass.position.set(0, 0.42, 0.015);
  g.add(cuirass);
  ...
  g.add(rivet);
  const strapA = box(0.13, 0.018, 0.024, leather);
  strapA.position.set(0, 0.48, 0.04);
  strapA.rotation.z = 0.55;
  g.add(strapA);
  const strapB = box(0.13, 0.016, 0.022, leather);
  strapB.position.set(0, 0.38, 0.045);
  strapB.rotation.z = -0.45;
  g.add(strapB);
  const collar = cyl(0.045, 0.05, 0.04, soot, 8);
  collar.position.y = 0.5;
  g.add(collar);
  const belt = box(0.13, 0.035, 0.07, scrap);
  belt.position.set(0, 0.3, 0.01);
  g.add(belt);
  const skirt = box(0.12, 0.1, 0.06, leather);
  skirt.position.set(0, 0.22, 0.02);
  g.add(skirt);
  const flap = box(0.05, 0.09, 0.025, soot);
  flap.position.set(0.02, 0.2, 0.05);
  g.add(flap);
  const flapL = box(0.045, 0.08, 0.022, scrap);
  flapL.position.set(-0.03, 0.2, 0.04);
  g.add(flapL);
  ...
  const arm = (s: 1 | -1, zRot: number, y: number) => {
    const a = cyl(0.02, 0.016, 0.16, leather, 6);
    a.position.set(s * 0.1, y, 0.02);
    a.rotation.z = zRot;
    g.add(a);
    const sleeve = cyl(0.026, 0.022, 0.1, soot, 6);
    sleeve.position.set(s * 0.09, y + 0.02, 0.015);
    sleeve.rotation.z = zRot;
    g.add(sleeve);
    const bracer = cyl(0.024, 0.022, 0.06, scrap, 6);
  ...
  const leg = (s: 1 | -1) => {
    const thigh = cyl(0.03, 0.024, 0.14, leather, 6);
    thigh.position.set(s * 0.045, 0.16, 0);
    g.add(thigh);
    const short = cyl(0.036, 0.03, 0.09, soot, 6);
    short.position.set(s * 0.045, 0.18, 0);
    g.add(short);
    const shin = cyl(0.022, 0.018, 0.12, leather, 6);
    shin.position.set(s * 0.048, 0.05, 0.01);
    g.add(shin);
    const greave = cyl(0.028, 0.024, 0.08, scrap, 6);
    greave.position.set(s * 0.05, 0.055, 0.02);
    g.add(greave);
    const wrap = box(0.045, 0.02, 0.045, leather);
