...
import { manhToTile, pickLanceOnTile, stepSmallToward, type SmallCell } from "./grid";
...
export function fireCannon(d: Dungeon, pilot: Pilot, tx: number, ty: number, fx: Fx[]) {
  ...
  const pick = pickLanceOnTile(lancePack(d), tx, ty, d.px, d.py);
  const foe = pick ? d.enemies.find((e) => e.id === pick.id && e.hp > 0) : undefined;
  if (!foe) {
    d.log = "No target.";
    return "ok" as const;
  }
...
export function shootCannon(d: Dungeon, pilot: Pilot, fx: Fx[]) {
  const locked = d.enemies.find((e) => e.id === d.lastAimId && e.hp > 0);
  if (locked && seenAt(d, locked.x, locked.y)) return fireCannon(d, pilot, locked.x, locked.y, fx);
  return shootRepeat(d, pilot, fx);
}
