...
function makeScythe(wood: THREE.Material, bladeM: THREE.Material, steel: THREE.Material, dark: THREE.Material) {
  ...
  for (let i = 0; i <= 16; i++) {
    const t = i / 16;
    const a = t * Math.PI * 0.88;
    pts.push(
      new THREE.Vector3(
        0.62 - Math.sin(a) * 0.2,
        -0.05 * Math.sin(a),
        0.05 + (1 - Math.cos(a)) * 0.4,
      ),
    );
  }
  const blade = new THREE.Mesh(
    new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), 16, 0.03, 5, false),
    bladeM,
  );
  g.add(blade);
  const edge = new THREE.Mesh(
    new THREE.TubeGeometry(
      new THREE.CatmullRomCurve3(pts.map((p) => p.clone().add(new THREE.Vector3(0, -0.01, 0.035)))),
      16,
      0.012,
      4,
      false,
