...
function makeScythe(wood: THREE.Material, bladeM: THREE.Material, steel: THREE.Material, dark: THREE.Material) {
  ...
  g.add(snath);
  const nib = (x: number) => {
    const n = cyl(0.011, 0.01, 0.1, wood, 6);
    n.position.set(x, 0.05, 0);
    g.add(n);
  };
  nib(-0.28);
  nib(0.22);
  const ring = cyl(0.018, 0.018, 0.032, steel, 8);
  ring.rotation.z = Math.PI / 2;
  ring.position.set(0.62, 0, 0);
  g.add(ring);
  add(g, box(0.035, 0.016, 0.028, dark), 0.6, 0, 0);

  const knife = new THREE.Shape();
  knife.moveTo(0, 0.003);
  knife.lineTo(0.018, 0.046);
  knife.quadraticCurveTo(0.26, 0.068, 0.56, 0.026);
  knife.quadraticCurveTo(0.58, 0.008, 0.59, 0);
  knife.quadraticCurveTo(0.28, -0.005, 0.016, 0.002);
  knife.closePath();
  const blade = new THREE.Mesh(
    new THREE.ExtrudeGeometry(knife, { depth: 0.006, bevelEnabled: false, steps: 1 }),
    bladeM,
  );
  blade.rotation.set(Math.PI / 2, 0, 0);
  blade.position.set(0.61, 0.003, 0.012);
  g.add(blade);
  return g;
}
