...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  let swinging = false;
  /**
   * Bind +Y is up. Hang = π. Elbow flex is **negative** (human crease in front, joint back).
   * Positive elbow was hyperextending (broken hinge). Scythe = hang + curl so both fists meet the snath at the hips.
   */
  const reap = rKind === "scythe";
  const hangU = Math.PI - 0.08;
  const hangE = -0.12;
  const presentU = Math.PI - 0.92;
  const presentE = -1.15;
  const restOf = (kind: string, side: 1 | -1) => {
    if (kind === "cannon") return { u: presentU, e: presentE, z: side * -0.18, y: side * -0.08 };
    return { u: hangU, e: hangE, z: side * -0.12, y: 0 };
  };
