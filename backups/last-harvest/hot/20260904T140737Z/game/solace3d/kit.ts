...
export function buildWanzer(kit: Kit): WanzerRig {
  ...
  legs.name = "legs";
  add(legs, box(0.3, 0.07, 0.2, dark), 0, hipY, 0);
  ...
  const leg = (side: number, rj: boolean) => {
    ...
    const foot = box(0.15, 0.05, 0.24, bootM);
    add(g, foot, 0, 0.025, 0.05);
    add(g, box(0.16, 0.032, 0.07, steel), 0, 0.02, -0.06);
  ...
  add(body, box(chestW * 0.82, 0.09, chestD * 0.92, plate), 0, hipY + 0.04, 0);
  add(body, box(chestW, 0.2, chestD, hatch), 0, hipY + 0.14, 0);
  add(body, box(0.12, 0.05, 0.02, dark), 0, hipY + 0.13, chestD / 2 + 0.012);
  const collar = cyl(0.1, 0.11, 0.045, dark, 8);
  add(body, collar, 0, neckY - 0.05, 0.02);
  add(body, box(0.34, 0.2, 0.2, packM), 0, hipY + 0.14, -chestD / 2 - 0.11);
  add(body, box(0.24, 0.055, 0.17, dark), 0, hipY + 0.25, -chestD / 2 - 0.11);
  add(body, box(0.09, 0.03, 0.03, yellow), 0, hipY + 0.26, -chestD / 2 - 0.2);
  ...
  if (kit.weapon === 0) {
    ...
    const gripL = new THREE.Vector3(-0.36, -0.3, 0.18);
    const gripR = new THREE.Vector3(0.4, -0.28, 0.14);
