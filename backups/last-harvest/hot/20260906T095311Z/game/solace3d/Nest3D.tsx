...
export function Nest3D({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const aimMark = new THREE.Mesh(
      new THREE.CylinderGeometry(0.1, 0.14, 0.02, 8),
      new THREE.MeshBasicMaterial({ color: 0xc9a227, transparent: true, opacity: 0.7, depthWrite: false }),
    );
    ...
    const plantAim = () => {
      ...
      aimMark.position.set(p.x, 0.04, p.z);
