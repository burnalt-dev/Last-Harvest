...
export function buildWanzer(kit: Kit): WanzerRig {
  ...
  const makeArm = (side: number) => {
    ...
    fist.position.set(0, -faLen - 0.02, 0.03);
    fore.add(fist);
    ...
    g.position.set(side * (chestW / 2 + 0.1), 0, 0.04);
  ...
  const idleG = (side: number) => new THREE.Euler(0.85, side * 0.1, side * 0.48);
  const reachG = (side: number) => new THREE.Euler(1.05, side * 0.14, side * 0.62);
  const idleFore = 0.28;
  const reachFore = 0.08;
  ...
  if (kit.weapon === 0) {
    ...
    scythe.position.set(0.02, -0.5, 0.14);
    scythe.rotation.set(0.2, 0.65, -0.18);
    swingRig.add(scythe);
  } else if (kit.weapon === 1) {
    add(wpn, box(0.035, 0.06, 0.24, bladeM), 0.14, -0.5, 0.12);
    add(wpn, box(0.05, 0.22, 0.14, vents), -0.18, -0.24, 0.03);
    swingRig.add(wpn);
  } else {
    const barrel = cyl(0.032, 0.048, 0.28, steel, 8);
    barrel.rotation.x = Math.PI / 2;
    add(wpn, barrel, 0.14, -0.44, 0.14);
    swingRig.add(wpn);
  }
  ...
  const swing = () => {
    ...
    const tick = (now: number) => {
      ...
      swingRig.rotation.set(0.18 * Math.sin(cut * Math.PI), 0.65 - cut * 1.8, 0.06 * (1 - cut));
