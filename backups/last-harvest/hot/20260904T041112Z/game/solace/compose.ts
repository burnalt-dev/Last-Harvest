...
export function idleDirsFor(loadout: Loadout): number[] {
  ...
  for (const id of ids) {
    ...
    if (!meta) continue;
    for (const f of meta.frames as unknown as { pose: string; dir: string }[]) {
...
function stackSockets(dest: CanvasRenderingContext2D, loadout: Loadout, dir: Dir, pose: string) {
  ...
  // S54: pistons + buckets in front of body from hip cut down
  const legsId = loadout.legs;
  const bodyId = loadout.body;
  const cut = (sockets as { legs_fore_y?: number }).legs_fore_y ?? 128;
  if (legsId && bodyId) {
    const meta = ATLASES[legsId as keyof typeof ATLASES];
    const bitmap = img(`parts/${legsId}`);
    const frames = meta?.frames as readonly { pose: string; dir: string; x: number; y: number; w: number; h: number }[] | undefined;
    const fr = frames?.find((f) => f.pose === pose && f.dir === dir) ?? frames?.find((f) => f.pose === "idle" && f.dir === dir);
    if (meta && bitmap && fr && cut < fr.h) {
      dest.drawImage(bitmap, fr.x, fr.y + cut, fr.w, fr.h - cut, 0, cut, fr.w, fr.h - cut);
    }
  }
  return n >= 3;
}
