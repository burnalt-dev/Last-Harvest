...
import { manhToTile, stepSmallToward, type SmallCell } from "./grid";
...
export function playerAct(
  ...
  if (foe) {
    ...
    if (d.gasketMode === "mount" || d.mounted) {
      ...
      const me: SmallCell = { x: d.cx, y: d.cy, sub: d.csub };
      if (manhToTile(me, d.px, d.py) > 2) {
        const next = stepSmallToward(me, { x: d.px, y: d.py, sub: 0 });
        if (walkableSmall(d, next, undefined, true)) {
          d.cx = next.x;
          d.cy = next.y;
          d.csub = next.sub;
        }
