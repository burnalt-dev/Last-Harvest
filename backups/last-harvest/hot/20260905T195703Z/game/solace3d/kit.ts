...
/** Baseline rest. Bay hold sliders write these; Copy sit dumps them. */
export const HOLD_PROFILES: Record<HoldId, HoldProfile> = {
  scytheR: { seat: 0, pitch: 0.22, roll: 0.18, yaw: -0.48 },
  scytheL: { seat: 0, pitch: 0.22, roll: -0.18, yaw: 0.48 },
  sawR: { seat: 0, pitch: 1.12, roll: 0, yaw: 0.22 },
  sawL: { seat: 0, pitch: 1.12, roll: 0, yaw: -0.22 },
  cannonR: { seat: 0.02, pitch: 0.12, roll: 0, yaw: 0 },
  cannonL: { seat: 0.02, pitch: 0.12, roll: 0, yaw: 0 },
  shieldR: { seat: 0, pitch: 0.45, roll: -0.12, yaw: 0.9 },
...
export function cloneSit(s: GoldSit = GOLD_SIT): GoldSit {
  ...
  for (const [k, v] of Object.entries(s.hold ?? HOLD_PROFILES)) hold[k as HoldId] = { ...v };
  return { ...s, hold };
}
