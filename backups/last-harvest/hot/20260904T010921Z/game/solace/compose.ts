...
function destOf(x: number, y: number, span: number, rec: { cx: number; foot: number; top: number }) {
  ...
  const h = Math.max(8, rec.foot - rec.top);
  const hasFeet = rec.foot > FOOT - 40 && h > 200;
  if (!hasFeet) {
    const dw = Math.max(96, (span * 220) | 0);
    const dh = dw;
    const cy = (rec.top + rec.foot) / 2;
    return {
      dw,
      dh,
  ...
  let k = targetH / Math.max(64, FOOT - rec.top);
  if (k > 1.1) k = 1.1;
  if (k < 0.92) k = 0.92;
  const dw = (base * k) | 0;
  const dh = dw;
