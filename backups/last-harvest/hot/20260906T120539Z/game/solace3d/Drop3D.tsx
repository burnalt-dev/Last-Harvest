  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (d.aiming && d.aimX >= 0 && d.aimY >= 0) {
        const tile = isoCell(d.aimX, d.aimY);
        aimMark.position.set(tile.x, 0, tile.z);
        aimMark.visible = true;
        const lock = d.enemies.find((e) => e.id === d.lastAimId && e.hp > 0 && e.x === d.aimX && e.y === d.aimY);
        if (lock) {
    ...
    const loop = () => {
      ...
      const muzz = muzzleOf();
      const aimOn = dNow.aiming && dNow.aimX >= 0 && dNow.aimY >= 0;
      const dest = aimOn
