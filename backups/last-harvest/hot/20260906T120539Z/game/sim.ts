...
export function walkable(d: Dungeon, x: number, y: number, ignore?: Enemy) {
  ...
  if (d.gasketMode !== "mount" && d.chp > 0 && d.cx === x && d.cy === y) return false;
  return true;
}

/** ¼-cell walk for Gasket + Stripekin. Full-tile units occupy every sub. Pilot blocks the diamond. */
export function walkableSmall(d: Dungeon, dest: SmallCell, ignore?: Enemy, skipGasket = false) {
  if (at(d, dest.x, dest.y) === 1) return false;
  if (d.px === dest.x && d.py === dest.y) return false;
...
export function lockAim(d: Dungeon, tx: number, ty: number) {
  ...
  stampAim(d, foe);
  d.log = `Lock · ${FOE_NAME[foe.kind]}`;
}

/** Enter Aim. Locks sticky/closest. Never damages. */
export function enterAim(d: Dungeon) {
  d.aiming = true;
  const foe = stickyLance(d) ?? closestLanceFoe(d);
  if (!foe) {
    d.log = "Aim: pick a tile.";
    return;
  }
  stampAim(d, foe);
  d.log = `Lock · ${FOE_NAME[foe.kind]}`;
...
export function exitAim(d: Dungeon) {
  d.aiming = false;
  d.log = "Aim off.";
}

/** Lancer Shoot is Pin: one body, one ¼. Never tile / arc / pierce. */
export const LANCER_PROJECTILE = "pin" as const;
/** Entity ids that took HP on the last Pin Shoot. Smoke: must be 0 or 1 unique id. */
export let lastPinHits: number[] = [];

function pinHit(d: Dungeon, foe: Enemy, dmg: number, fx: Fx[]) {
  lastPinHits.push(foe.id);
  foe.hp -= dmg;
  foe.seen = true;
  foe.ai = "alert";
  d.lastAimId = foe.id;
...
export function fireCannon(d: Dungeon, pilot: Pilot, tx: number, ty: number, fx: Fx[], round = true) {
  if (d.waitCannon) {
    d.log = "Lance cycling.";
    return "ok" as const;
  }
  const sticky = stickyLance(d);
  const onTile = sticky && sticky.x === tx && sticky.y === ty ? sticky : primaryOnTile(d, tx, ty);
  const foe = onTile;
  if (!foe) {
    d.log = "No target.";
    return "ok" as const;
  ...
  return lanceStrike(d, pilot, foe, fx, round);
}

/** Pin: exactly one primary. Never tile-splash / never other ¼ on the same square. */
function lanceStrike(d: Dungeon, pilot: Pilot, foe: Enemy, fx: Fx[], round: boolean) {
  ...
  d.waitCannon = true;
  stampAim(d, foe);
  const primaryId = foe.id;
  const only = d.enemies.find((e) => e.id === primaryId && e.hp > 0);
  if (!only) {
    d.waitCannon = false;
    d.log = "No target.";
    return "ok" as const;
  ...
  const snap = d.enemies.map((e) => ({ id: e.id, hp: e.hp }));
  if (dual) {
    let total = 0;
    let hits = 0;
    const raw = st.melee * 0.5;
    for (let i = 0; i < 4; i++) {
      fx.push({ k: "bolt", x0: d.px, y0: d.py, x1: only.x, y1: only.y, t: 0.16 + i * 0.07, side: i % 2 === 0 ? 1 : -1 });
      if (Math.random() > st.acc) continue;
      hits += 1;
      const dmg = Math.max(0, Math.round(raw - only.def));
      total += dmg;
      pinHit(d, only, dmg, fx);
    }
    if (hits === 0) {
      d.log = "Pin miss.";
      fx.push({ k: "banner", text: "MISS", good: false, t: 0.45 });
    } else {
      fx.push({ k: "banner", text: `PIN ${total}`, good: total > 0, t: 0.5 });
      d.log = total > 0 ? `Pin · ${total}` : "Pin ate the armor.";
      if (only.hp <= 0) {
        const drop = rollKill(only.kind);
        if (drop) {
          add(d.bag, drop);
          d.ore = d.bag.r004 ?? 0;
          d.scrap = d.bag.r001 ?? 0;
        }
        d.log = drop === "r004" ? `${FOE_NAME[only.kind]} down · bloom-ore` : drop === "r001" ? `${FOE_NAME[only.kind]} down · splinter` : `${FOE_NAME[only.kind]} down.`;
      }
    }
  } else {
    const miss = Math.random() > st.acc;
    fx.push({ k: "bolt", x0: d.px, y0: d.py, x1: only.x, y1: only.y, t: 0.35, side: guns.l && !guns.r ? -1 : 1 });
    if (miss) {
      d.log = "Pin miss.";
      fx.push({ k: "banner", text: "MISS", good: false, t: 0.45 });
    } else {
      const dmg = applyDef(st.melee, only.def, 0);
      pinHit(d, only, dmg, fx);
      fx.push({ k: "banner", text: `PIN ${dmg}`, good: true, t: 0.45 });
      d.log = `Pin · ${FOE_NAME[only.kind]}`;
      if (only.hp <= 0) {
        const drop = rollKill(only.kind);
        if (drop) {
          add(d.bag, drop);
  ...
  for (const s of snap) {
    ...
    const e = d.enemies.find((x) => x.id === s.id);
    if (e) e.hp = s.hp;
  }
  lastPinHits = lastPinHits.filter((id) => id === primaryId);
  if (typeof console !== "undefined") {
    const ids = [...new Set(lastPinHits)];
    console.log(`PIN_DMG ids=${ids.join(",") || "none"} class=${LANCER_PROJECTILE}`);
  }
  if (!round) {
    d.waitCannon = false;
    return "ok";
...
function primaryOnTile(d: Dungeon, tx: number, ty: number): Enemy | null {
  const sticky = stickyLance(d);
  if (sticky && sticky.x === tx && sticky.y === ty) return sticky;
  const p = pickLanceOnTile(lancePack(d), tx, ty, d.px, d.py);
  return p ? d.enemies.find((e) => e.id === p.id && e.hp > 0) ?? null : null;
}
...
/** Aim-tile: Pin the one occupant. No-Aim: closest global. Empty tile = no-op (no neighbor splash). */
export function shootCannon(d: Dungeon, pilot: Pilot, fx: Fx[], round = true) {
  if (d.waitCannon) {
    d.log = "Lance cycling.";
    return "ok" as const;
  }
  const aimed = d.aiming && d.aimX >= 0 && d.aimY >= 0;
  const foe = aimed ? primaryOnTile(d, d.aimX, d.aimY) : stickyLance(d) ?? closestLanceFoe(d);
  if (!foe) {
    d.log = "No target.";
