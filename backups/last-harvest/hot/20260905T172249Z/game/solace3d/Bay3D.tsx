...
import { buildWanzer, clampKit, GOLD_GREY, GOLD_REV, GOLD_SIT, KIT_NAMES, type GoldSit, type Kit, type KitSlot } from "./kit";
...
const SIT_SLIDERS: {
  ...
  { key: "gripSize", label: "gripper", min: 0.5, max: 1.8, step: 0.02, when: (k) => n(k, "handR") === "scythe" || n(k, "handR") === "saw" || n(k, "handL") === "shield" },
...
export function Bay3D({ onBack }: { onBack: () => void }) {
  ...
  const cycle = (slot: KitSlot) => {
    setKit((k) => {
      const two = KIT_NAMES.handR[k.handR] === "scythe";
      if (two && (slot === "handL" || slot === "handR")) {
        const nextR = (k.handR + 1) % KIT_NAMES.handR.length;
        return clampKit({ ...k, handR: nextR, handL: 0 });
      }
