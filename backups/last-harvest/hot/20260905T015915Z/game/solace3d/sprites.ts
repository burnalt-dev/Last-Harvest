...
/** Ship-hanger iso diamond. Same 2:1 blit as dungeon cells. */
export function hangerCell(kind: "pad" | "plate" | "grate") {
  ...
  if (kind === "pad") {
    ...
    ctx.lineWidth = 1;
    ctx.strokeRect(cx - 18, cy - 10, 36, 20);
  }
  if (kind === "grate") {
  ...
  ctx.restore();
  ctx.strokeStyle = kind === "pad" ? "#c9a227" : "rgba(230, 220, 180, 0.28)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(cx, cy - hh);
  ctx.lineTo(cx + hw, cy);
