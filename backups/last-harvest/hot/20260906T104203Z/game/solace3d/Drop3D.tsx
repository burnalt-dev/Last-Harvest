...
import { makeAimLock, makePulseOrb, spawnPulse, tickAimLock, tickPulses, type PulseShot } from "./pulse";
...
export function Drop3D({
  ...
  useEffect(() => {
    ...
    const muzzleGlowR = makePulseOrb();
    const muzzleGlowL = makePulseOrb();
    muzzleGlowR.scale.setScalar(0.35);
    muzzleGlowL.scale.setScalar(0.35);
    muzzleGlowR.visible = false;
    muzzleGlowL.visible = false;
    scene.add(muzzleGlowR, muzzleGlowL);
    ...
    const syncFoes = (d: Dungeon) => {
      ...
      for (const e of d.enemies) {
        ...
        const locked = d.lastAimId === e.id;
        g.scale.setScalar(locked ? 1.12 : 1);
    ...
    const sync = () => {
      ...
      if (d.lastAimId >= 0) {
        const lock = d.enemies.find((e) => e.id === d.lastAimId && e.hp > 0);
        if (lock) {
          const p = lock.kind === "small" ? subCell(lock.x, lock.y, lock.sub ?? 0) : isoCell(lock.x, lock.y);
          aimMark.position.set(p.x, 0, p.z);
    ...
    const loop = () => {
      ...
      const dest = aimOn
        ...
        : { x: 0, z: 0 };
      const glow = (orb: THREE.Group, tip: THREE.Object3D | null) => {
        if (!aimOn || !tip) {
          orb.visible = false;
          return;
        }
        tip.getWorldPosition(tmpA);
        orb.position.copy(tmpA);
        const wob = 0.32 + Math.sin(now / 140) * 0.06;
        orb.scale.setScalar(wob);
        orb.visible = true;
      };
      glow(muzzleGlowR, muzz.R);
      glow(muzzleGlowL, muzz.L);
      if (aimOn) tickAimLock(aimMark, now);
      for (const [id, g] of foes) {
        const locked = aimOn && id === dNow.lastAimId;
        const fit = g.userData.fitScale ?? g.scale.x;
        if (g.userData.fitScale === undefined) g.userData.fitScale = fit;
        g.scale.setScalar(fit * (locked ? 1.14 + Math.sin(now / 140) * 0.04 : 1));
      }
      for (const f of fxRef.current) {
        ...
        seenBolt.add(id);
        const tip = (f.side ?? 1) < 0 ? muzz.L ?? muzz.R : muzz.R ?? muzz.L;
        const end =
          dNow.lastAimId >= 0 && dNow.aimX === f.x1 && dNow.aimY === f.y1
            ? subCell(f.x1, f.y1, dNow.aimSub >= 0 ? dNow.aimSub : 0)
            : isoCell(f.x1, f.y1);
        if (tip) {
          tip.getWorldPosition(tmpA);
          spawnPulse(pulses, livePulses, tmpA.x, tmpA.y, tmpA.z, end.x, 0.26, end.z, 240);
        } else {
          const start = isoCell(f.x0, f.y0);
          spawnPulse(pulses, livePulses, start.x, 0.45, start.z, end.x, 0.26, end.z, 240);
        }
      }
