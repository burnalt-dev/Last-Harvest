...
import { hasTwoHandMelee, isArmed, equipWeapon, starterLoadout, kitOf } from "./parts";
import { DIFFICULTY, FOE_NAME, FOE_TEACH, HUB_STANCES, STANCE_HUB, type Difficulty, type Stance } from "./data";
...
type Mode = "title" | "ng-diff" | "ng-stance" | "ship" | "dungeon" | "dead" | "lookdev" | "bay3d" | "nest";
...
export function LastHarvest() {
  ...
  const [hubStance, setHubStance] = useState<Stance>("twohand");
  ...
  const drop = (who: Pilot = pilot) => {
    if (!isArmed(who.loadout) || boarding.current) return;
    ...
    void loadPlay().then((p) => {
      setPilot(who);
      setDungeon(p.makeDungeon("forest", 1, who));
      setMode("dungeon");
      setDropCam("far");
      setVerb(null);
  ...
  const onTile = useCallback(
    (tx: number, ty: number) => {
      ...
      if (dungeon.aiming) {
        void loadPlay().then((p) => {
          p.lockAim(dungeon, tx, ty);
          setPicked(dungeon.lastAimId >= 0 ? dungeon.lastAimId : null);
          setDungeon({ ...dungeon });
        });
        return;
      }
        ...
        {live ? (
          <div className="relative z-[80] mt-3 grid gap-1">
            <Plate
              ...
              onClick={() => {
                boarding.current = false;
                setHubStance("twohand");
                setPilot(applyDifficulty(newPilot(), "default"));
                setMode("ng-diff");
              }}
            >
              New game
  ...
  if (mode === "ng-diff") {
    return (
      <main className="relative z-[80] flex h-dvh flex-col bg-bg p-4" data-qa="ng-diff">
        <p className="engraved text-xs">New game · 1/2</p>
        <h2 className="mt-2 text-3xl font-bold">How hard?</h2>
        <p className="mt-3 text-sm leading-snug text-muted">
          This drop is difficult by design. If you don’t want that stress, pick Story.
        </p>
        <div className="relative z-[80] mt-4 grid gap-2">
          <DifficultyDock value={pilot.difficulty} onPick={(id) => setPilot(applyDifficulty(pilot, id))} />
          <Plate qa="ng-diff-next" onClick={() => setMode("ng-stance")}>
            Next
          </Plate>
          <Plate onClick={() => setMode("title")}>Back</Plate>
        </div>
      </main>
    );
  }
  ...
  if (mode === "ng-stance") {
    const card = STANCE_HUB[hubStance];
    return (
      <main className="relative z-[80] flex h-dvh flex-col bg-bg p-4" data-qa="ng-stance">
        <p className="engraved text-xs">New game · 2/2</p>
        <h2 className="mt-2 text-3xl font-bold">Fit a stance</h2>
        <p className="mt-2 text-sm text-muted">Pilot kit only. Back bus included — no mesh.</p>
        <div className="relative z-[80] mt-3 grid min-h-0 flex-1 grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2">
          {HUB_STANCES.map((id) => {
            const s = STANCE_HUB[id];
            return (
              <Plate
                key={id}
                qa={`ng-stance-${id}`}
                className={hubStance === id ? "text-cyan" : ""}
                onClick={() => setHubStance(id)}
              >
                <span className="block text-lg font-bold">{s.name}</span>
                <span className="mt-1 block text-sm text-muted">{s.line}</span>
                <span className="mt-1 block text-[11px] text-brass">{s.hands} · back included</span>
              </Plate>
            );
          })}
        </div>
        <p className="mt-2 text-[11px] text-muted">{card.hands} · {card.bonus}</p>
        <div className="relative z-[80] mt-2 grid gap-2">
          <Plate
            qa="ng-stance-board"
            onClick={() => {
              const loadout = equipWeapon(starterLoadout(), hubStance);
              drop({ ...pilot, weapon: hubStance, loadout, skillgrams: ["sg001", "sg002"] });
            }}
          >
            Board · {card.name}
          </Plate>
          <Plate onClick={() => setMode("ng-diff")}>Back</Plate>
        </div>
      </main>
    );
  }
  ...
  if (mode === "dungeon" && dungeon) {
    ...
    const stance = kitOf(pilot.loadout);
    const canAim = stance === "lancer" || stance === "lancers" || stance === "sawlance";
    const canShoot = stance === "lancer" || stance === "lancers";
    const sgSlots: (string | "")[] = [pilot.skillgrams[0] ?? "", pilot.skillgrams[1] ?? ""];
    return (
        ...
        <div className="drop-dock" data-qa="drop-dock">
          <div className="flex items-center gap-2">
            ...
            <p className="drop-log ml-2 min-w-0 flex-1 truncate" data-qa="drop-log">
              {dungeon.aiming && dungeon.lastAimId < 0 ? "Lance: pick a tile" : dungeon.log}
            </p>
          ...
          <div className="mt-1 flex items-end gap-2">
            ...
            <div className="grid min-w-0 flex-1 grid-cols-2 gap-2">
              ...
              {canShoot ? (
                <Plate
                  qa="drop-shoot"
                  className={verb === "aim" && dungeon.lastAimId >= 0 ? "text-cyan" : ""}
                  onClick={() => {
                    setVerb(null);
                    void loadPlay().then((p) => afterAct(p.shootCannon(dungeon, pilot, fx)));
                  }}
                >
                  Shoot
                </Plate>
              ...
              {sgSlots.map((id, i) => {
                const sg = id ? SKILLGRAMS[id] : undefined;
                return (
                  <Plate
                    key={`sg-${i}`}
                    qa={`drop-sg-${i}`}
                    ...
                    onClick={() => {
                      if (!sg) return;
                      void loadPlay().then((p) => afterAct(p.fireSkillgram(dungeon, pilot, fx, i as 0 | 1)));
                    }}
                  >
                    {sg ? sg.name : "Empty"}
                  </Plate>
          ...
          <div className="flex items-center gap-2">
            <p className="drop-hint min-w-0 flex-1" data-qa="drop-hint">
              {verb === "attack"
                ? "Wide 3 · tiles ahead"
                : verb === "harvest"
                  ? "Adjacent bloom"
                  : dungeon.aiming
                    ? dungeon.lastAimId >= 0
                      ? "Shoot confirms the lock"
                      : "Aim: pick a tile · pad faces"
                    : "Move bump-strikes. End Turn passes."}
            </p>
            <Plate qa="drop-program" className="!px-2 !py-1 text-[10px]" onClick={() => setMenu(menu === "sheet" ? null : "sheet")}>
              Deck
            </Plate>
          </div>
