...
import { buildWanzer, GOLD_GREY, GOLD_SIT } from "./kit";
import { buildGasket } from "./gasket";
import { buildFoe } from "./foe3d";
import { isoCell, ISO_YAW, isoDiamondGeo, loadIsoTile, sliceSheet } from "./sprites";
import type { Dungeon } from "../sim";

/** FM 3/4 follow. Far = tactical; close = melee ring. Radar slate owns R5. */
export const DROP_CAM = { far: 11, close: 16 } as const;
const FRUSTUM = 22;
...
export function Drop3D({
  ...
  useEffect(() => {
    ...
    camera.position.set(2.4, 5.8, 7.4);
    ...
    const followCam = (x: number, z: number) => {
      camera.position.set(x + 2.4, 5.8, z + 7.4);
      camera.lookAt(x, 0.14, z);
    };
