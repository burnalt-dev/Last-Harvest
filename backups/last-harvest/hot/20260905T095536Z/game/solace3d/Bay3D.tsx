...
const CAM_ZOOM = { min: 0.4, max: 30, step: 0.05, def: 12 };
const BAY = 5;
...
const SIT_SLIDERS: { key: keyof GoldSit; label: string; min: number; max: number; step: number }[] = [
  ...
  { key: "thighLen", label: "thigh len", min: 0.1, max: 0.36, step: 0.01 },
  { key: "shinLen", label: "shin len", min: 0.08, max: 0.32, step: 0.01 },
  { key: "stance", label: "stance W", min: 0.06, max: 0.28, step: 0.01 },
...
export function Bay3D({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const followCam = (x: number, z: number) => {
      // 3/4 from south-east, not a second 45° on XZ (that + diamond pack crabs).
