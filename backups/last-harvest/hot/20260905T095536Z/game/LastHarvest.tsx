...
import { preloadBiomeOrGo, preloadBoot } from "./art";
...
const DROP_CAM = { far: 11, close: 16 } as const;
    ...
    return (
      <main className="relative z-[80] flex h-dvh flex-col bg-bg p-4" data-live={live ? "1" : "0"}>
        <article className="title-card">
          ...
          <p className="title-talent">You have the team — do you have the talent?</p>
        </article>
        {live ? (
          <div className="relative z-[80] mt-4 grid gap-2">
            <p className="engraved text-xs">Fit a harvest kit</p>
    ...
    return (
      <main className="relative flex h-dvh flex-col bg-bg">
        <Suspense fallback={<div className="min-h-0 w-full flex-1 bg-bg text-muted">Seating Veldt…</div>}>
          <Drop3D dungeon={dungeon} onTile={onTile} onStep={stepPad} zoom={DROP_CAM[dropCam]} />
        </Suspense>
        {fx
        ...
        <div className="absolute right-2 top-3 z-20 flex flex-col items-end gap-2">
          ...
          <Plate qa="drop-cam" className="!px-2 !py-2 text-center text-[10px] tracking-widest uppercase" onClick={() => setDropCam(dropCam === "far" ? "close" : "far")}>
            {dropCam === "far" ? "Close" : "Far"}
          </Plate>
