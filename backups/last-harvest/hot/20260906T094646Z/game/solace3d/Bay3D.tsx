...
import { applyHeavyLabor, buildWanzer, clampKit, cloneSit, cycleHands, dumpHoldProfiles, GOLD_GREY, GOLD_REV, GOLD_SIT, HOLD_IDS, holdId, KIT_NAMES, plantY, resetGoldGlobals, resetHoldProfiles, setBonesVisible, setSeatGizmoVisible, type GoldSit, type HoldId, type HoldProfile, type Kit, type KitSlot } from "./kit";
...
const SIT_SLIDERS: {
  ...
  { key: "upperLen", label: "upper arm", min: 0.1, max: 0.95, step: 0.01 },
  { key: "faLen", label: "forearm", min: 0.1, max: 1.05, step: 0.01 },
  { key: "thick", label: "arm thick", min: 0.03, max: 0.22, step: 0.002 },
  { key: "shoulderW", label: "shoulder W", min: 0.08, max: 0.4, step: 0.005 },
  { key: "hipsW", label: "hips W", min: 0.06, max: 0.36, step: 0.005 },
  ...
  { key: "footSize", label: "foot size", min: 0.03, max: 0.16, step: 0.005 },
  { key: "torsoLean", label: "torso lean", min: -0.25, max: 0.35, step: 0.01 },
...
export function Bay3D({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const pulseR = mkBeam(0xc9a227, 1);
    const pulseL = mkBeam(0xc9a227, 1);
    ...
    api.current = {
      ...
      shoot: () => {
        pulseUntil = performance.now() + 320;
      },
    };
    ...
    const loop = () => {
      ...
      const muzz = cannonMuzzles(wanzer.root);
      const look = DIR[facing] ?? DIR[0]!;
      const end = isoCell(cellPos.tx + look.x * 3, cellPos.ty + look.y * 3);
      const now = performance.now();
      const pulsing = now < pulseUntil;
      const showAim = aimingLive || pulsing;
      const place = (line: THREE.Line, tip: THREE.Object3D | null, hot: boolean, opacity: number) => {
        if (!hot || !tip) {
          line.visible = false;
          return;
        }
        tip.getWorldPosition(tmpA);
        writeLine(line, tmpA.x, tmpA.y, tmpA.z, end.x, 0.22, end.z);
        (line.material as THREE.LineBasicMaterial).opacity = opacity;
        line.visible = true;
      };
      place(aimBeamR, muzz.R, showAim, 0.45 + Math.sin(now / 160) * 0.12);
      place(aimBeamL, muzz.L, showAim, 0.45 + Math.sin(now / 160) * 0.12);
      const pu = pulsing ? Math.max(0, 1 - (now - (pulseUntil - 320)) / 320) : 0;
      place(pulseR, muzz.R, pulsing, pu);
      place(pulseL, muzz.L, pulsing, pu);
      if (moving) {
      ...
      body: (
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {n(kit, "handR") === "cannon" || n(kit, "handL") === "cannon" ? (
              <>
                <Plate
                  qa="bay3d-aim"
                  ...
                  onClick={() => {
                    ...
                    if (next) sfxScope();
                    setAiming(next);
                    api.current?.setAim(next);
                    if (next) pickView("scope");
                  }}
                >
                  Aim
                </Plate>
      ...
      body: (
        <div>
          <div className="mb-2 flex gap-2">
            ...
            <Plate qa="bay3d-sit-heavy" onClick={() => setSit((s) => applyHeavyLabor(s))}>
              Sit: Heavy labor
            </Plate>
