...
import { buildGasket, GASKET_SIT } from "./gasket";
import { buildFoe, tickFoe } from "./foe3d";
...
export function Drop3D({
  ...
  useEffect(() => {
    ...
    const matUnseen = solidMat(0x1a1612);
    const matSeen = solidMat(0x2a241c);
    const matFloor = solidMat(0x8a6e4a);
    const matVariant = solidMat(0x6a5844);
    const matUndergrowth = solidMat(0x565044);
    const matStairs = solidMat(0x2a1c14);
    const mats = {
      unseen: matUnseen,
      seen: matSeen,
      floor: matFloor,
      variant: matVariant,
      under: matUndergrowth,
      stairs: matStairs,
    };
    ...
    scene.add(lasers);
    const liveLasers: { line: THREE.Line; t0: number; life: number; side: number; x1: number; y1: number }[] = [];
    const seenBolt = new Set<string>();
    const mkBeam = (color: number, opacity: number) => {
      const geo = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(), new THREE.Vector3(0, 0.2, 1)]);
    ...
    const aimBeamR = mkBeam(0x5ee0d0, 0.55);
    const aimBeamL = mkBeam(0x5ee0d0, 0.55);
    const muzzleOf = () => cannonMuzzles(wanzer.root);
    const tmpA = new THREE.Vector3();
    const tmpB = new THREE.Vector3();
    const writeLine = (line: THREE.Line, ax: number, ay: number, az: number, bx: number, by: number, bz: number) => {
    ...
    gasket.scale.setScalar(GASKET_SIT.scale);
    scene.add(gasket);
    const aimMark = new THREE.Mesh(
      new THREE.CylinderGeometry(0.12, 0.16, 0.04, 10),
      new THREE.MeshBasicMaterial({ color: 0x5ee0d0, transparent: true, opacity: 0.7, depthWrite: false }),
    );
    aimMark.name = "aimLock";
    ...
    const matForTile = (tile: number) =>
      tile === 2 ? mats.stairs : tile === 3 ? mats.variant : tile === 1 ? mats.under : mats.floor;
    ...
    const placeGasket = (d: Dungeon, yaw: number) => {
      ...
      if (d.mounted) {
        ...
        gasket.scale.setScalar(0.24);
      } else {
        if (gasket.parent) gasket.parent.remove(gasket);
        scene.add(gasket);
        const p = subCell(d.cx, d.cy, d.csub ?? 0);
    ...
    const syncFoes = (d: Dungeon) => {
      ...
      for (const e of d.enemies) {
        ...
        g.userData.faceYaw = ISO_YAW[e.dir] ?? 0;
        g.userData.foeState = e.ai ?? "idle";
        if (g.userData.faceSnap) {
          g.rotation.y = g.userData.faceYaw;
          g.userData.faceSnap = false;
        }
      }
    ...
    const sync = () => {
      ...
      if (d.aiming && d.aimX >= 0 && d.aimY >= 0) {
        const p = subCell(d.aimX, d.aimY, d.aimSub >= 0 ? d.aimSub : 0);
        aimMark.position.set(p.x, 0.03, p.z);
    ...
    const loop = () => {
      ...
      for (const g of foes.values()) tickFoe(g, now);
      wanzer.root.updateMatrixWorld(true);
      const dNow = dunRef.current;
      const muzz = muzzleOf();
      const aimOn = dNow.aiming && dNow.aimX >= 0 && dNow.aimY >= 0 && dNow.lastAimId >= 0;
      const dest = aimOn
        ? subCell(dNow.aimX, dNow.aimY, dNow.aimSub >= 0 ? dNow.aimSub : 0)
        : { x: 0, z: 0 };
      const placeBeam = (line: THREE.Line, tip: THREE.Object3D | null) => {
        if (!aimOn || !tip) {
          line.visible = false;
          return;
        }
        tip.getWorldPosition(tmpA);
        tmpB.set(dest.x, 0.22, dest.z);
        writeLine(line, tmpA.x, tmpA.y, tmpA.z, tmpB.x, tmpB.y, tmpB.z);
        (line.material as THREE.LineBasicMaterial).opacity = 0.45 + Math.sin(performance.now() / 160) * 0.12;
        line.visible = true;
      };
      placeBeam(aimBeamR, muzz.R);
      placeBeam(aimBeamL, muzz.L);
      for (const f of fxRef.current) {
        ...
        seenBolt.add(id);
        const geo = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(), new THREE.Vector3(0, 0.2, 1)]);
        const line = new THREE.Line(geo, new THREE.LineBasicMaterial({ color: 0xc9a227, transparent: true, opacity: 1, depthWrite: false }));
        line.frustumCulled = false;
        lasers.add(line);
        liveLasers.push({ line, t0: performance.now(), life: 280, side: f.side ?? 0, x1: f.x1, y1: f.y1 });
      }
      const nowL = performance.now();
      for (let i = liveLasers.length - 1; i >= 0; i--) {
        const L = liveLasers[i]!;
        const u = (nowL - L.t0) / L.life;
        const tip = L.side < 0 ? muzz.L ?? muzz.R : muzz.R ?? muzz.L;
        const end =
          aimOn && dNow.aimX === L.x1 && dNow.aimY === L.y1
            ? dest
            : isoCell(L.x1, L.y1);
        if (tip) {
          tip.getWorldPosition(tmpA);
          writeLine(L.line, tmpA.x, tmpA.y, tmpA.z, end.x, 0.22, end.z);
        }
    ...
    void (async () => {
      const [floorTex, fogTex, treeA, treeB, anvil, spark] = await Promise.all([
        sliceSheet("/game/tiles/forest.png?v=veldt1", 2, 2).catch(() => [] as THREE.CanvasTexture[]),
      ...
      if (floorTex[1]) mats.under = texMat(floorTex[1]);
