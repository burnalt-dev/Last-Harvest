...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  let swinging = false;
  /** Bind has arm +Y up. Hang = π (wrist Y world down). Lance present = elbow −π/2 (forearm at camera). */
  ...
  const rest = {
    chestX: 0,
    upperL: lKind === "cannon" ? presentU : hangU,
    upperR: rKind === "cannon" ? presentU : hangU,
    elbowL: lKind === "cannon" ? presentE : hangE,
    elbowR: rKind === "cannon" ? presentE : hangE,
    zL: lKind === "cannon" ? 0.12 : 0.06,
    zR: rKind === "cannon" ? -0.12 : -0.06,
  };
  ...
  if (rKind === "scythe") armR.wrist.rotation.z = -0.08;
  ...
  const swing = () => {
    ...
    const tickSw = (now: number) => {
      ...
      armL.upper.rotation.x = rest.upperL - lift * 0.55;
