...
function stripekin(): THREE.Group {
  ...
  g.add(belly);
  const strap = box(0.09, 0.012, 0.02, scrap);
  strap.position.set(0, 0.44, 0.045);
  strap.rotation.z = 0.4;
  g.add(strap);
  const loincloth = box(0.08, 0.07, 0.03, soot);
  loincloth.position.set(0, 0.24, 0.03);
  g.add(loincloth);

  const pauldron = (s: 1 | -1) => {
    const p = box(0.07, 0.035, 0.08, scrap);
    p.position.set(s * 0.08, 0.48, 0);
    p.rotation.z = s * 0.25;
    g.add(p);
    const spike = box(0.012, 0.04, 0.012, soot);
    spike.position.set(s * 0.09, 0.52, 0.01);
    g.add(spike);
  };
  pauldron(1);
  pauldron(-1);
