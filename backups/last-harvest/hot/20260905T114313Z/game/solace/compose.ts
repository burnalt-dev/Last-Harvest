...
function stackSockets(dest: CanvasRenderingContext2D, loadout: Loadout, dir: Dir, pose: string) {
  ...
  // S54 sandwich is body-rear → legs → body-fore (fauld flaps).
  // Packed as same body id, extra frames pose `idle` + `layer: "fore"`.
  // Until those frames exist, do not blit legs over the skirt.
  return n >= 1;
}
