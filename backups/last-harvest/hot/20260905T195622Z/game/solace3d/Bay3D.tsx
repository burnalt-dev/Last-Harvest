...
import { buildWanzer, clampKit, cloneSit, cycleHands, GOLD_GREY, GOLD_REV, GOLD_SIT, holdId, KIT_NAMES, plantY, setBonesVisible, setSeatGizmoVisible, type GoldSit, type HoldProfile, type Kit, type KitSlot } from "./kit";
...
const SIT_SLIDERS: {
  ...
  { key: "holdAngle", label: "hold angle", min: -0.8, max: 1.2, step: 0.02 },
  { key: "weaponSeat", label: "weapon seat", min: 0, max: 0.12, step: 0.005 },
...
const HOLD_FIELDS: { key: keyof HoldProfile; label: string; min: number; max: number; step: number }[] = [
  { key: "seat", label: "seat", min: -0.08, max: 0.12, step: 0.005 },
  { key: "pitch", label: "pitch", min: -1.6, max: 1.6, step: 0.02 },
  { key: "roll", label: "roll", min: -1.6, max: 1.6, step: 0.02 },
  { key: "yaw", label: "yaw", min: -1.6, max: 1.6, step: 0.02 },
];
...
function HoldSliders({
  ...
  const rows: { id: ReturnType<typeof holdId>; label: string }[] = [];
  if (rKind !== "none") rows.push({ id: holdId(rKind, 1), label: `hold R · ${rKind}` });
  if (lKind !== "none" && rKind !== "scythe") rows.push({ id: holdId(lKind, -1), label: `hold L · ${lKind}` });
  if (!rows.length) return null;
  const patch = (id: ReturnType<typeof holdId>, key: keyof HoldProfile, v: number) => {
    setSit((s) => ({
      ...s,
    ...
    <div className="mt-2 border-t border-border pt-2">
      {rows.map((row) => (
        <div key={row.id} className="mb-1">
          <p className="engraved mb-1 text-[10px]">{row.label}</p>
          <div className="grid grid-cols-1 gap-x-4 gap-y-1 sm:grid-cols-2">
            {HOLD_FIELDS.map((f) => {
              const val = sit.hold[row.id]?.[f.key] ?? 0;
              return (
                <label key={f.key} className="flex items-center gap-2 text-[11px] text-fg">
                  <span className="w-24 shrink-0 uppercase tracking-wide text-brass">{f.label}</span>
                  <input
                    type="range"
                    min={f.min}
                    max={f.max}
                    step={f.step}
                    value={val}
                    onChange={(e) => patch(row.id, f.key, Number(e.target.value))}
      ...
      body: (
        <div>
          <div className="mb-2 flex gap-2">
            <Plate qa="bay3d-sit-reset" onClick={() => setSit(cloneSit())}>
              Reset gold
            </Plate>
            <Plate
              qa="bay3d-sit-copy"
