import { BIOMES, STORY, applyDef, statsOf, type BiomeId, type Difficulty } from "./data";
...
function bumpTurn(d: Dungeon, pilot: Pilot, keepCannon = false) {
  ...
  /** Story: Recovery heals HP 1:1 (high companion Wits). Default: trickle. */
  const heal = d.difficulty === "story" ? rec : rec * 0.15;
  d.php = Math.min(d.pmax, d.php + heal);
  reveal(d, pilot);
}
