...
import { preloadBiomeOrGo, preloadBoot, preloadLoadout } from "./art";
import { sfxMove, sfxUi } from "./audio";
import { DIR, faceTo, fromIso, iso } from "./iso";
import { equipWeapon, hasTwoHandMelee, isArmed, isRanged, type Loadout } from "./parts";
...
let playP: Promise<Play> | null = null;
function loadPlay() {
  playP ??= import("./play");
  return playP;
}

type Mode = "title" | "ship" | "dungeon" | "dead" | "lookdev" | "mechtest" | "bay3d";
...
export function LastHarvest() {
  ...
  useEffect(() => {
    ...
    let lastW = 0;
    let lastH = 0;
    let draw: Play["drawDungeon"] | null = null;
    ...
    const loop = () => {
      ...
      if (d && draw) {
        ...
        draw(ctx, d, pilot.loadout, fx, null);
      }
  ...
  const onCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
      ...
      if (!c) return;
      const r = c.getBoundingClientRect();
      const p0 = "touches" in e ? e.touches[0] : e;
      if (!p0) return;
      ...
      const t = fromIso(p0.clientX - r.left, p0.clientY - r.top, ox, oy);
      if (dungeon.aiming) {
        void loadPlay().then((p) => afterAct(p.fireCannon(dungeon, pilot, t.tx, t.ty, fx) ?? "ok"));
        return;
      }
      const dx = Math.sign(t.tx - dungeon.px);
      const dy = Math.sign(t.ty - dungeon.py);
      if (dx === 0 && dy === 0) return;
      stepPad(faceTo(dungeon.px, dungeon.py, t.tx, t.ty));
    },
    [dungeon, fx, pilot],
  );

  const pickKit = (w: WeaponId) => {
      ...
      <main className="relative z-[80] flex h-dvh flex-col bg-bg p-4" data-live={live ? "1" : "0"}>
        ...
        {live ? (
          <div className="relative z-[80] mt-4 grid gap-2">
            ...
            <Plate qa="title-mechtest" onClick={() => setMode("mechtest")}>
              Mech Test
            </Plate>
        ...
        {live && isArmed(pilot.loadout) && <TitleMech loadout={pilot.loadout} />}
      </main>
    );
  }

  if (mode === "mechtest") {
    return <MechParts onBack={() => setMode("title")} />;
  }
  ...
  if (mode === "lookdev") {
    return <LookdevBoard onBack={() => setMode("title")} />;
  }
...
function LookdevBoard({ onBack }: { onBack: () => void }) {
  ...
  return (
    <main className="relative z-[80] flex h-dvh flex-col overflow-auto bg-bg p-4">
      <p className="engraved text-xs">Lookdev · not the dungeon</p>
      <h1 className="mt-2 text-3xl font-bold">{roster?.pass ?? "Concepts"}</h1>
      <div className="mt-4 grid gap-4">
        {roster?.sheets.map((s) => (
          <figure key={s.file}>
            <img src={`/game/lookdev/${s.file}`} alt={s.label} className="w-full rounded-sm border border-gold/40" style={{ imageRendering: "pixelated" }} />
            <figcaption className="mt-1 font-mono text-xs text-muted">{s.label}</figcaption>
          </figure>
        ))}
      </div>
      ...
      <Plate className="mt-6" onClick={onBack}>
        Title
      </Plate>
    </main>
  );
