...
export function buildWanzer(kit: Kit): WanzerRig {
  ...
  let gripL = new THREE.Vector3(-0.42, -0.32, 0.36);
  let gripR = new THREE.Vector3(0.44, -0.3, 0.32);
  if (kit.weapon === 0) {
  ...
  const swing = () => {
    ...
    const tick = (now: number) => {
      ...
      if (u < 0.16) {
        ...
        const c = (u - 0.16) / 0.42;
        yaw = 0.72 - c * 1.5;
        pitch = 0.55 + Math.sin(c * Math.PI) * 0.12;
        flash = Math.sin(c * Math.PI);
        coil = 1 - c * 2;
        lean = 0.18 + Math.sin(c * Math.PI) * 0.2;
        dip = Math.sin(c * Math.PI) * 0.04;
        lunge = 0.16 + Math.sin(c * Math.PI) * 0.42;
      } else {
        const r = (u - 0.58) / 0.42;
        yaw = -0.78 * (1 - r);
        pitch = 0.55 * (1 - r);
      ...
      swingRig.position.set(0, shoulderY, 0.14 + lunge);
      swingRig.rotation.set(pitch, yaw, 0.04 * Math.sin(u * Math.PI));
      body.position.z = lunge * 0.4;
      body.rotation.set(bodyIdleX + lean, yaw * 0.42 + coil * 0.12, coil * -0.14);
      legs.rotation.set(lean * 0.2, yaw * 0.16 + coil * 0.18, coil * -0.06);
      legs.position.y = -dip;
      head.rotation.set(lean * 0.4, yaw * 0.2, 0);
      reach(L, gripL, -1);
      reach(R, gripR, 1);
