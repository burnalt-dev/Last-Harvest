import { BIOMES, applyDef, derive, type BiomeId } from "./data";
import { DIR, faceTo } from "./iso";
import { starterLoadout, meleeAttack, cannonHands, hasCellPack, type Loadout } from "./parts";
...
export function fireCannon(d: Dungeon, pilot: Pilot, tx: number, ty: number, fx: Fx[]) {
  ...
  if (guns.r && guns.l && !hasCellPack(pilot.loadout)) {
    d.log = "One bus — need cell pack to dual.";
  }
  if (dual) {
    let total = 0;
    let hits = 0;
