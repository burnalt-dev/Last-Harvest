...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  if (rKind === "scythe") {
    armR.wrist.add(buildGripper(1, "pole"));
    armL.wrist.add(buildGripper(-1, "pole"));
  } else {
    if (rKind === "saw") armR.wrist.add(buildGripper(1, "hilt"));
    if (lKind === "shield") armL.wrist.add(buildGripper(-1, "slab"));
  }
  ...
  let swinging = false;
  /** Bind +Y is up. Hang = π. Lance present = elbow −π/2. Scythe = hip farmer-cut (both fists on the snath). */
  ...
  const rest = {
    chestX: 0,
    upperL: reap ? 2.2 : lKind === "cannon" ? presentU : hangU,
    upperR: reap ? 2.25 : rKind === "cannon" ? presentU : hangU,
    elbowL: reap ? 1.05 : lKind === "cannon" ? presentE : hangE,
    elbowR: reap ? 1.1 : rKind === "cannon" ? presentE : hangE,
    zL: reap ? 0.2 : lKind === "cannon" ? 0.12 : 0.06,
    zR: reap ? -0.2 : rKind === "cannon" ? -0.12 : -0.06,
  };
  ...
  if (reap) {
    ...
    if (pole && mountL && mountR) {
      const local = mountR.worldToLocal(mountL.getWorldPosition(new THREE.Vector3()));
      if (local.lengthSq() > 1e-6) {
        pole.quaternion.setFromUnitVectors(new THREE.Vector3(1, 0, 0), local.clone().normalize().negate());
