...
export function kitOf(loadout: Loadout) {
  ...
  if (r === "p018" || (r === "p009" && l === "p009")) return "scythe";
  if (r === "p010" && l === "p010") return hasCellPack(loadout) ? "dual" : "cannon";
  if (r === "p008" && l === "p010") return "sawgun";
  if (r === "p010" || l === "p010") return "cannon";
  if (r === "p008") return "blade";
  return null;
}

/** Dual-draw backpack. Lore: power bus. Required to fire two cannons. */
export function hasCellPack(loadout: Loadout) {
  return loadout.body === "p020";
}
...
export function equipWeapon(loadout: Loadout, weapon: WeaponId): Loadout {
  if (weapon === "scythe") return { ...loadout, handL: "p018", handR: "p018" };
  if (weapon === "cannon") return { ...loadout, handL: "p007", handR: "p010" };
  if (weapon === "dual") return { ...loadout, handL: "p010", handR: "p010", body: "p020" };
  if (weapon === "sawgun") return { ...loadout, handL: "p010", handR: "p008" };
