...
function makeScythe(wood: THREE.Material, bladeM: THREE.Material, steel: THREE.Material, dark: THREE.Material) {
  ...
  const steps = 14;
  hook.moveTo(ox + Math.cos(a0) * inner, oy + Math.sin(a0) * inner);
  for (let i = 0; i <= steps; i++) {
    const a = a0 + (a1 - a0) * (i / steps);
    hook.lineTo(ox + Math.cos(a) * outer, oy + Math.sin(a) * outer);
  }
  for (let i = steps; i >= 0; i--) {
    const a = a0 + (a1 - a0) * (i / steps);
    hook.lineTo(ox + Math.cos(a) * inner, oy + Math.sin(a) * inner);
  }
  hook.closePath();
  const blade = new THREE.Mesh(
    new THREE.ExtrudeGeometry(hook, { depth: 0.012, bevelEnabled: false, steps: 1 }),
    bladeM,
  );
  blade.name = "blade";
  blade.rotation.set(Math.PI / 2, 0, 0);
  blade.position.set(0.58, 0.006, 0);
  g.add(blade);
  return g;
}
...
export function buildWanzer(kit: Kit): WanzerRig {
  ...
  if (scythe) {
    const bladeMesh = scythe.getObjectByName("blade") as THREE.Mesh;
    glow = bladeMesh.clone();
    glow.material = slashMat;
    glow.name = "slash";
    glow.scale.set(1.07, 1.07, 2.6);
    glow.visible = false;
    scythe.add(glow);
  }
  const clearGhosts = () => {
  ...
  const stamp = () => {
    ...
    const ghost = glow.clone();
    ghost.material = slashMat.clone();
    (ghost.material as THREE.MeshBasicMaterial).opacity = 0.5;
    ghost.matrixAutoUpdate = false;
    inv.copy(root.matrixWorld).invert();
    ghost.matrix.copy(inv).multiply(glow.matrixWorld);
  ...
  const swing = () => {
    ...
    const tick = (now: number) => {
      ...
      if (glow) {
        glow.visible = flash > 0.02;
        slashMat.opacity = 0.2 + flash * 0.75;
      }
      if (flash > 0.12 && now - lastStamp > 26) {
        stamp();
        lastStamp = now;
      }
      fadeGhosts();
