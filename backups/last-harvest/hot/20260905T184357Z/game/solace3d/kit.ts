...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  const seatR = gripR.getObjectByName("weapon_seat") ?? gripR;
  const seatL = gripL.getObjectByName("weapon_seat") ?? gripL;
  seatR.add(wpnSlotR);
  seatL.add(wpnSlotL);
  ...
  if (!reap) {
    ...
    const slab = wpnSlotL.getObjectByName("shield");
    if (slab && lKind === "shield") slab.rotation.set(-1.05, -0.18, 0.08);
  }
  if (reap) {
