...
/** Universal clamp on `weapon_mount_*`. Tools snap into `weapon_seat`. Mirrored L/R. */
export function buildGripper(side: 1 | -1, sit: GoldSit): THREE.Group {
  ...
  const digit = (yaw: number, splay: number, scale = 1) => {
    ...
    root.position.set(splay * sit.gripWidth, 0.02, 0);
  ...
  seat.name = "weapon_seat";
  seat.position.y = 0.018 + sit.weaponSeat;
  g.add(seat);
  return g;
}
...
/** Tool-only forearm lance. Seats in the universal grip — no baked hand. Muzzle along +Y. */
function buildCannon(sit: GoldSit, _side: 1 | -1 = 1): THREE.Group {
  ...
  gun.name = "cannon";
  const collar = cyl(r * 1.05, r * 0.95, len * 0.12, dark, 8);
  ...
  ring.position.y = len * 0.22;
  gun.add(ring);
  const ring2 = cyl(r * 1.06, r * 1.06, len * 0.07, yellow, 8);
  ring2.position.y = len * 0.62;
  gun.add(ring2);
