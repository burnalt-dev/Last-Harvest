...
import { buildWanzer, KIT_NAMES, type Kit, type KitSlot } from "./kit";
import { buildGasket } from "./gasket";
import { loadBillboard, loadIsoTile, isoCell } from "./sprites";

const DIRS = ["S", "E", "N", "W"] as const;

export function Bay3D({ onBack }: { onBack: () => void }) {
  const host = useRef<HTMLDivElement>(null);
  const [kit, setKit] = useState<Kit>({ head: 0, body: 0, legs: 0, arms: 0, weapon: 0 });
  ...
  useEffect(() => {
    ...
    scene.add(ground);
    // Camera is +X+Z. Side tiles (0,-1) and (0,1) stay out of the crate's silhouette.
    const ORC = isoCell(0, -1);
    const GOBS = isoCell(0, 1);
    void (async () => {
      const proto = await loadIsoTile("/game/tiles/forest.png", 1.15);
      (proto.material as THREE.SpriteMaterial).depthWrite = false;
  ...
  return (
    <main className="relative z-[80] flex h-dvh flex-col bg-bg">
      <div className="flex items-center justify-between px-3 py-2">
        <p className="engraved text-xs">3D bay · slim crate · two-hand scythe</p>
      ...
      <div className="grid grid-cols-3 gap-2 p-3 sm:grid-cols-4">
        ...
        <Plate onClick={() => api.current?.swing()}>Swing</Plate>
