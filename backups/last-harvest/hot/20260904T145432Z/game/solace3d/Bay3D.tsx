      ...
      for (let ty = -1; ty <= 1; ty++) {
        ...
        const fill = new THREE.Mesh(
          ...
          new THREE.MeshLambertMaterial({
            color: home ? 0x3a444c : odd ? 0x2a3238 : 0x242c32,
          }),
        );
        fill.rotation.x = -Math.PI / 2;
        fill.position.set(cell.x, 0, cell.z);
        floor.add(fill);
        const edge = new THREE.LineLoop(
          edgeGeo,
          new THREE.LineBasicMaterial({ color: home ? 0x8aa0b0 : 0x4a5860 }),
        );
        edge.position.set(cell.x, 0, cell.z);
        floor.add(edge);
      }
    }
