...
import { attachArm, attachArmShell, attachLeg, attachLegShell, buildBody, buildBodyShell, buildGripper, buildHead, buildHeadShell, buildPack, buildWeapon } from "./parts3d";
...
/** Armor `none` = default gunmetal shells (not raw bones, not dressed Solace). Scythe both hands. */
...
export type GoldSit = {
  ...
  shieldH: number;
  shieldW: number;
  /** Per-weapon rest on `tool_seat`. Global grip sit still nudges the whole assembly. */
  hold: Record<HoldId, HoldProfile>;
};

export type HoldProfile = { seat: number; pitch: number; roll: number; yaw: number };
export type HoldId = `${"scythe" | "saw" | "cannon" | "shield"}${"R" | "L"}`;

export const HOLD_ZERO: HoldProfile = { seat: 0, pitch: 0, roll: 0, yaw: 0 };

export const HOLD_IDS: HoldId[] = [
  "scytheR",
  "scytheL",
  "sawR",
  "sawL",
...
/** Per-weapon×hand rest on `tool_seat`. L is authored, never negate-R. */
export const HOLD_PROFILES: Record<HoldId, HoldProfile> = {
  scytheR: { seat: 0, pitch: 0.22, roll: 0.18, yaw: -0.48 },
  scytheL: { seat: 0.01, pitch: 0.16, roll: 0.06, yaw: -0.22 },
  sawR: { seat: 0, pitch: 1.12, roll: 0, yaw: 0.22 },
  sawL: { seat: 0, pitch: 1.05, roll: -0.08, yaw: -0.18 },
  cannonR: { seat: 0.02, pitch: 0.12, roll: 0, yaw: 0 },
  cannonL: { seat: 0.02, pitch: 0.1, roll: 0.04, yaw: 0.06 },
  shieldR: { seat: 0, pitch: 0.4, roll: 0.1, yaw: 0.85 },
  shieldL: { seat: 0, pitch: 0.45, roll: 0.12, yaw: -0.9 },
};

export function dumpHoldProfiles(sit: GoldSit): string {
  const lines = HOLD_IDS.map((id) => {
    const h = sit.hold?.[id] ?? HOLD_PROFILES[id] ?? HOLD_ZERO;
    return `${id} hold_seat=${h.seat} hold_pitch=${h.pitch} hold_roll=${h.roll} hold_yaw=${h.yaw}`;
  });
  return `HOLD PROFILES\n${lines.join("\n")}`;
}

export function holdId(kind: string, side: 1 | -1): HoldId {
  return `${kind}${side < 0 ? "L" : "R"}` as HoldId;
}

export function holdOf(kind: string, side: 1 | -1, sit: GoldSit): HoldProfile {
  const id = holdId(kind, side);
  const h = sit.hold?.[id];
  return { ...HOLD_ZERO, ...(HOLD_PROFILES[id] ?? HOLD_ZERO), ...h };
}

export function applyHold(obj: THREE.Object3D, kind: string, side: 1 | -1, sit: GoldSit) {
  /* Layer 2: per-weapon×hand on the tool. Globals live on the grip / tool_seat (layer 3). */
  const h = holdOf(kind, side, sit);
  obj.position.set(0, h.seat, 0);
  obj.rotation.set(h.pitch, h.yaw, h.roll);
}

export function cloneSit(s: GoldSit = GOLD_SIT): GoldSit {
  const hold = {} as Record<HoldId, HoldProfile>;
  for (const id of HOLD_IDS) hold[id] = { ...(s.hold?.[id] ?? HOLD_PROFILES[id] ?? HOLD_ZERO) };
  return { ...s, hold };
}

/** Restore GOLD_SIT globals. Keep live per-weapon hold profiles. */
export function resetGoldGlobals(live: GoldSit): GoldSit {
  const hold = {} as Record<HoldId, HoldProfile>;
  for (const id of HOLD_IDS) hold[id] = { ...(live.hold?.[id] ?? HOLD_PROFILES[id] ?? HOLD_ZERO) };
  return { ...GOLD_SIT, hold };
}

/** Restore HOLD_PROFILES only. Global sit stays. */
export function resetHoldProfiles(live: GoldSit): GoldSit {
  const hold = {} as Record<HoldId, HoldProfile>;
  for (const id of HOLD_IDS) hold[id] = { ...(HOLD_PROFILES[id] ?? HOLD_ZERO) };
  return { ...live, hold };
}

/** Body/global sit for industrial silhouette. Does not touch hold profiles. */
export const SIT_HEAVY: Pick<
  GoldSit,
  | "chestW"
  | "chestD"
  | "chestH"
...
export function applyHeavyLabor(live: GoldSit): GoldSit {
  const hold = {} as Record<HoldId, HoldProfile>;
  for (const id of HOLD_IDS) hold[id] = { ...(live.hold?.[id] ?? HOLD_PROFILES[id] ?? HOLD_ZERO) };
  return { ...live, ...SIT_HEAVY, hold };
}
...
export const GOLD_REV = 9;

/** Slot 0 = none → default shell. Fancy plates swap the shell. Hands stay armed. */
...
/** Hide gold rods + brass balls. Sockets/sit still drive transforms. */
export function setBonesVisible(root: THREE.Object3D, on: boolean) {
  root.traverse((o) => {
    if (o.userData.rigVis || o.name === "rigVis" || o.name === "ribs") o.visible = on;
  });
}

/** Bay-only `tool_seat` axes. Play hides them. */
export function setSeatGizmoVisible(root: THREE.Object3D, on: boolean) {
  root.traverse((o) => {
    if (o.userData.bayGizmo || o.name === "tool_seat_gizmo") o.visible = on;
  });
}
export function plantY(sit: GoldSit) {
  const hang = 0.03 + sit.thighLen + 0.02 + sit.shinLen + 0.03;
  const ball = (0.02 + sit.thick * 0.08) * 0.9;
  return Math.max(0, hang + ball + 0.012 - sit.hipY) * sit.scale;
}
...
export function buildWanzer(kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  /** Elbow / wrist / knee / ankle — ~2× spine nodes, readable under Bones + weapons. */
  const rLimb = (0.046 + sit.thick * 0.14) * 2;

  const hips = joint("hips", 0, boneM, brass, rB, rJ * 1.15);
  hips.position.y = hipY;
  root.add(hips);

  const skirt = new THREE.Mesh(new THREE.TorusGeometry(sit.hipsW * 0.5, 0.006, 6, 16), brass);
  skirt.name = "skirt_ring";
  skirt.userData.rigVis = true;
  ...
  spine_low.rotation.x = sit.torsoLean;
  hips.add(spine_low);
  ...
  const makeArm = (side: "L" | "R") => {
    const s = side === "L" ? -1 : 1;
    const clav = joint(`clavicle_${side}`, sit.shoulderW, boneM, brass, rB * 0.9, rJ * 0.85);
    ...
    shoulder.position.y = sit.shoulderW;
    ...
    wrist.add(wmount);
    return { shoulder, upper, elbow, forearm, wrist, wmount };
  };
  ...
  const makeLeg = (side: "L" | "R") => {
    ...
    hip.position.set(s * sit.hipsW * 0.5, 0, 0);
    hip.rotation.x = Math.PI;
    const splay = (sit.stance - sit.hipsW) * 0.5;
    hip.rotation.z = s * Math.atan2(splay, Math.max(0.08, thighLen + shinLen));
    hips.add(hip);
    ...
    shin.add(ankle);
    const foot = joint(`foot_${side}`, sit.footSize, boneM, brass, rB * 0.9, rJ * 0.9);
  ...
  if (headKind === "none") headSlot.add(buildHeadShell(sit));
  else headSlot.add(buildHead(headKind, sit));
  if (bodyKind === "none") bodySlot.add(buildBodyShell(sit));
  else {
    bodySlot.add(buildBody(bodyKind === "cell" ? "can" : bodyKind, sit));
    pack_mount.add(buildPack(bodyKind, sit));
  }
  if (armsKind === "none") {
    attachArmShell(armL, sit, -1);
    attachArmShell(armR, sit, 1);
  } else {
    attachArm(armL, armsKind, sit, -1);
    attachArm(armR, armsKind, sit, 1);
  }
  if (legsKind === "none") {
    attachLegShell(legL, sit, -1);
    attachLegShell(legR, sit, 1);
  } else {
    attachLeg(legL, legsKind, sit, -1);
    attachLeg(legR, legsKind, sit, 1);
  }
  const gripR = buildGripper(1, sit);
  const gripL = buildGripper(-1, sit);
  armR.wmount.add(gripR);
  armL.wmount.add(gripL);
  const seatR = gripR.getObjectByName("tool_seat") ?? gripR;
  const seatL = gripL.getObjectByName("tool_seat") ?? gripL;
  wpnSlotR.position.set(0, 0, 0);
  wpnSlotR.rotation.set(0, 0, 0);
  wpnSlotR.scale.set(1, 1, 1);
  wpnSlotL.position.set(0, 0, 0);
  wpnSlotL.rotation.set(0, 0, 0);
  wpnSlotL.scale.set(1, 1, 1);
  seatR.add(wpnSlotR);
  seatL.add(wpnSlotL);
  if (rKind !== "none") {
    wpnSlotR.add(buildWeapon(rKind, sit, 1));
    applyHold(wpnSlotR, rKind, 1, sit);
  }
  if (lKind !== "none" && rKind !== "scythe") {
    wpnSlotL.add(buildWeapon(lKind, sit, -1));
    applyHold(wpnSlotL, lKind, -1, sit);
  }
  ...
  let swinging = false;
  /**
   * Bind +Y is up. Elbow flex is **negative** (human crease in front).
   * Scythe = farmer-cut at the hips. 1H = combat ready (saw guard, slab up, lance present).
   */
  const reap = rKind === "scythe";
  const presentU = Math.PI - 0.92;
  const presentE = -1.15;
  const restOf = (kind: string, side: 1 | -1) => {
    if (kind === "cannon") return { u: presentU, e: presentE, z: side * -0.2, y: side * -0.1, wX: 0.12, wY: 0, wZ: side * 0.08 };
    if (kind === "saw") return { u: Math.PI - 0.48, e: -1.02, z: side * -0.18, y: side * -0.04, wX: 0.78, wY: 0.12, wZ: side * 0.12 };
    if (kind === "shield") return { u: Math.PI - 0.62, e: -0.92, z: side * -0.12, y: 0.1, wX: 0.35, wY: side * 0.25, wZ: 0 };
    return { u: Math.PI - 0.08, e: -0.12, z: side * -0.12, y: 0, wX: 0, wY: 0, wZ: 0 };
  };
  ...
  const rest = {
    chestX: 0,
    upperL: reap ? 2.8 : L.u,
    upperR: reap ? 2.82 : R.u,
    elbowL: reap ? -1.28 : L.e,
    elbowR: reap ? -1.26 : R.e,
    yL: reap ? 0.1 : L.y,
    yR: reap ? -0.1 : R.y,
    zL: reap ? 0.26 : L.z,
    zR: reap ? -0.26 : R.z,
  };
  armL.upper.rotation.set(rest.upperL, rest.yL, rest.zL);
  armR.upper.rotation.set(rest.upperR, rest.yR, rest.zR);
  armL.elbow.rotation.x = rest.elbowL;
  armR.elbow.rotation.x = rest.elbowR;
  if (!reap) {
    armL.wrist.rotation.set(L.wX, L.wY, L.wZ);
    armR.wrist.rotation.set(R.wX, R.wY, R.wZ);
  }
  ...
  const tick = (now: number) => {
    ...
    hips.position.y = hipY + Math.max(0, s) * 0.008;
  ...
  return { root, mount, swing, tick, plantY: plantY(sit) };
}
