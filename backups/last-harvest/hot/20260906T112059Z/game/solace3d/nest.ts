...
import { faceSmall, manhSmall, manhToTile, smallCellTaken, stepSmall, stepSmallToward, type SmallCell } from "../grid";
...
export function nestRound(actors: NestActor[], gasket: NestGasket, now: number, cleave = false, hasPilot = false) {
  ...
  const world: AiWorld = {
    ...
    blocked: (who, dest) => {
      ...
      const occ = [
        ...actors
          .filter((a) => a.hp > 0 && String(a.uid) !== who.id)
          .map((a) => ({ x: a.x, y: a.y, sub: a.sub, small: a.small })),
        ...(who.id !== "gasket" && gasket.hp > 0 ? [{ x: gasket.x, y: gasket.y, sub: gasket.sub, small: true }] : []),
      ];
      if (who.occ === "tile") return occ.some((o) => o.x === dest.x && o.y === dest.y);
      return smallCellTaken(occ, dest);
    },
...
/** Shoot = Pin. Drop shootCannon, no dungeon tickRound. One primary only — extras restored. */
export function nestShoot(actors: NestActor[], g: NestGasket, lance: NestLance, fx: Fx[]) {
  const snap = actors.map((a) => ({ uid: a.uid, hp: a.hp }));
  const d = nestDungeon(actors, g, lance);
  const pilot = nestPilot(lance.stance);
  shootCannon(d, pilot, fx, false);
  pullLance(d, actors, lance);
  const keep = lance.lastAimId;
  const hit: number[] = [];
  for (const s of snap) {
    const a = actors.find((n) => n.uid === s.uid);
    if (!a) continue;
    if (s.uid !== keep) a.hp = s.hp;
    else if (a.hp !== s.hp) hit.push(s.uid);
  }
  const extra = lastPinHits.filter((id) => id !== keep);
  if (extra.length) {
    for (const id of extra) {
      const s = snap.find((n) => n.uid === id);
      const a = actors.find((n) => n.uid === id);
      if (s && a) a.hp = s.hp;
    }
  }
  if (hit.length > 1) {
    for (const id of hit.slice(1)) {
      const s = snap.find((n) => n.uid === id);
      const a = actors.find((n) => n.uid === id);
      if (s && a) a.hp = s.hp;
    }
  }
  if (!lance.log.startsWith("Pin") && !lance.log.includes("down") && hit.length === 1) {
    lance.log = `Pin · #${hit[0]}`;
  }
  return lance;
}
