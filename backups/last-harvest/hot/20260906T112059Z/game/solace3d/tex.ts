...
export type TexKind = "plate" | "hatch" | "vents" | "stripe" | "visor" | "pack" | "dark" | "steel" | "boot" | "wood" | "blade" | "yellow" | "gasket" | "cream" | "brass" | "charcoal" | "rubber" | "hide" | "pelt" | "stone";
...
function paint(kind: TexKind) {
  return canvas(32, 32, (ctx, w, h) => {
    ...
    if (kind === "hide") {
      ...
      for (let i = 0; i < 5; i++) ctx.fillRect(0, 3 + i * 6, w, 3);
...
export function addOutline(root: THREE.Object3D, inflate = 1.07) {
  ...
  root.traverse((o) => {
    ...
    if (mesh.userData.bayGizmo || mesh.parent?.userData.bayGizmo || mesh.parent?.name === "tool_seat_gizmo") return;
    const ol = new THREE.Mesh(mesh.geometry, outlineMat);
    ol.scale.setScalar(inflate);
    ol.userData.outline = true;
    if (mesh.userData.rigVis) ol.userData.rigVis = true;
    extras.push(ol);
    mesh.add(ol);
  });
}
