...
import { isoCell, hangerTile } from "./sprites";
import { DIR } from "../iso";
...
export function Bay3D({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const protos = {
      pad: hangerTile("pad"),
      plate: hangerTile("plate"),
      grate: hangerTile("grate"),
    };
    const edgePts = [
      new THREE.Vector3(0, 0.006, 0.5),
      new THREE.Vector3(0.5, 0.006, 0),
      new THREE.Vector3(0, 0.006, -0.5),
      new THREE.Vector3(-0.5, 0.006, 0),
    ];
    ...
    for (let tx = -BAY; tx <= BAY; tx++) {
      for (let ty = -BAY; ty <= BAY; ty++) {
        ...
        tile.material = (protos[kind].material as THREE.MeshLambertMaterial).clone();
        const cell = isoCell(tx, ty);
        tile.position.set(cell.x, 0, cell.z);
        floor.add(tile);
        const line = new THREE.LineLoop(
          edgeGeo,
          new THREE.LineBasicMaterial({ color: kind === "pad" ? 0xc9a227 : 0x8a8476 }),
        );
        line.position.set(cell.x, 0, cell.z);
        floor.add(line);
      }
    }
