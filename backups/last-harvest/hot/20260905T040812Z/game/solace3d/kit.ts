...
/** Tapered ribcage: wide shoulders, slim waist. Not a crate box. FM industrial, humanoid. */
function ribs(topW: number, botW: number, h: number, topD: number, botD: number, mat: THREE.Material) {
  const g = new THREE.Group();
  g.name = "ribs";
  const hoop = (rw: number, rd: number, y: number) => {
    const m = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.0065, 5, 18), mat);
  ...
  const botY = -h * 0.5;
  hoop(topW, topD, topY);
  hoop((topW + botW) * 0.5, (topD + botD) * 0.5, 0);
  hoop(botW, botD, botY);
  ...
  for (const [sx, sz] of [
    ...
    const x0 = sx * topW * 0.36;
    const z0 = sz * topD * 0.36;
    const x1 = sx * botW * 0.36;
    const z1 = sz * botD * 0.36;
    dir.set(x1 - x0, botY - topY, z1 - z0);
    const len = dir.length();
    const post = new THREE.Mesh(new THREE.CylinderGeometry(0.0055, 0.0055, len, 5), mat);
    post.position.set((x0 + x1) * 0.5, (topY + botY) * 0.5, (z0 + z1) * 0.5);
    post.quaternion.setFromUnitVectors(up, dir.normalize());
    g.add(post);
  }
  const sternum = new THREE.Mesh(new THREE.CylinderGeometry(0.007, 0.009, h * 0.88, 6), mat);
  sternum.position.set(0, 0, topD * 0.34);
  g.add(sternum);
  return g;
}
...
export function buildWanzer(_kit: Kit, sit: GoldSit = GOLD_SIT): WanzerRig {
  ...
  const skirt = new THREE.Mesh(new THREE.TorusGeometry(sit.chestW * 0.2, 0.007, 6, 16), brass);
