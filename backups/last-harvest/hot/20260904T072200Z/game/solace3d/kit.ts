...
export function buildWanzer(kit: Kit): WanzerRig {
  ...
  if (kit.weapon === 0) {
    ...
    const gripL = new THREE.Vector3(-0.12, -0.46, 0.17);
    const gripR = new THREE.Vector3(0.2, -0.44, 0.15);
  ...
  const swing = () => {
    ...
    const tick = (now: number) => {
      ...
      const cut = u < 0.15 ? 0 : u > 0.85 ? 0 : Math.sin(((u - 0.15) / 0.7) * Math.PI);
      // Hands stay on the snath; the rig is the harvest sweep.
      swingRig.rotation.set(0.12 * cut, 0.55 - cut * 1.55, 0.05 * (1 - cut));
