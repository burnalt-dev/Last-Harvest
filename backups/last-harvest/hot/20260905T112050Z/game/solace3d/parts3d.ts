...
function buildScythe(sit: GoldSit): THREE.Group {
  ...
  scythe.add(blade);
  scythe.rotation.set(Math.PI, 0.22, 0.1);
  scythe.position.set(0.02 + sit.grip * 0.05, 0.02, 0.02);
  g.add(scythe);
  return g;
}
...
function buildShield(side: 1 | -1 = 1): THREE.Group {
  ...
  door.position.set(side * 0.08, 0.08, 0.05);
  slab.add(door);
  const rim = box(0.18, 0.44, 0.014, steel);
  rim.position.set(side * 0.08, 0.08, 0.032);
  slab.add(rim);
  const slot = box(0.05, 0.028, 0.012, dark);
  slot.position.set(side * 0.08, 0.2, 0.07);
  slab.add(slot);
  const warn = box(0.036, 0.036, 0.012, yellow);
  warn.position.set(side * 0.08, -0.04, 0.07);
  slab.add(warn);
  const grip = cyl(0.016, 0.016, 0.08, dark, 6);
  grip.position.set(side * 0.02, 0.04, 0.02);
  slab.add(grip);
  slab.rotation.set(0.05, side * -0.35, side * 0.08);
  g.add(slab);
  return g;
}
