...
import { loadPilot, probeWitsKeys, writePilot, WITS_KEY_PROBE } from "./save";
...
export function LastHarvest() {
  ...
  const [witsProbe, setWitsProbe] = useState("");
  ...
  useEffect(() => {
    ...
    if (loaded) {
      setPilot(loaded);
      setWitsProbe(probeWitsKeys("load-save", loaded));
    } else {
      const fresh = newPilot();
      setPilot(fresh);
      setWitsProbe(probeWitsKeys("new-game", fresh));
    }
    setHydrated(true);
  }, []);
  ...
  if (mode === "title") {
    return (
      <main className="relative z-[80] flex h-dvh flex-col bg-bg p-4" data-live={live ? "1" : "0"}>
        {WITS_KEY_PROBE && witsProbe ? (
          <p className="pointer-events-none absolute left-2 top-1 z-[90] max-w-[min(100%,24rem)] font-mono text-[9px] leading-snug text-muted" data-qa="wits-probe">
            {witsProbe}
          </p>
        ) : null}
      ...
      <main className="relative flex h-dvh flex-col bg-bg" data-qa="drop-hud">
        ...
        {fx
          ...
          .map((f, i) => (
            <div
              key={`${f.text}-${i}`}
              className="pointer-events-none absolute left-1/2 top-[22%] -translate-x-1/2 text-2xl font-bold"
              style={{ color: f.good ? "#5ee0d0" : "#9a4030" }}
            >
              {f.text}
            </div>
          ))}
        <div className="pointer-events-none absolute left-3 top-3 font-mono text-[11px] text-cyan">
          Veldt-9 · 1/1
          <div className="max-w-[14rem] text-muted">{dungeon.log}{dungeon.aiming ? " · LANCE AIM" : ""}</div>
        </div>
        ...
        <div className="drop-dock" data-qa="drop-dock">
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1" data-qa="gasket-pip">
              ...
              {(["follow", "hold", "focus"] as const).map((c) => (
                <Plate
                  key={c}
                  qa={`drop-gasket-${c}`}
          ...
          <div className="mt-1 flex items-end gap-2">
            ...
            <div className="grid min-w-0 flex-1 grid-cols-2 gap-2">
              {hasTwoHandMelee(pilot.loadout) ? (
                <Plate qa="drop-attack" onClick={() => void loadPlay().then((p) => afterAct(p.swingAttack(dungeon, pilot, fx)))}>
                  Attack
                </Plate>
              ...
              <Plate qa="drop-harvest" onClick={() => void loadPlay().then((p) => afterAct(p.harvestHere(dungeon, pilot, fx)))}>
                Harvest
              </Plate>
          ...
          <Plate qa="drop-wait" className="drop-end mt-2 text-cyan" onClick={() => void loadPlay().then((p) => afterAct(p.waitTurn(dungeon, pilot, fx)))}>
            End Turn
          </Plate>
        </div>
        {menu === "return" && (
